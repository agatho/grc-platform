...
export async function POST(req: Request) {
  ...
  const hashedIdentifier = hashDataSubjectIdentifier(body.data.dataSubjectIdentifier, orgSalt);

  const created = await withAuditContext(ctx, async (tx) => {
    const [item] = await tx.insert(consentRecord).values({
      orgId: ctx.orgId,
  ...
  return Response.json({ data: { id: created.id, consentGivenAt: created.consentGivenAt } }, { status: 201 });
}
