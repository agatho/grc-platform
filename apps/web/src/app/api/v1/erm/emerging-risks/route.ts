...
import { createEmergingRiskSchema, updateEmergingRiskSchema } from "@grc/shared";
...
export async function GET(req: Request) {
  ...
  const [items, [{ value: total }]] = await Promise.all([
    db.select().from(emergingRisk).where(where)
      .orderBy(desc(emergingRisk.createdAt)).limit(limit).offset(offset),
...
export async function POST(req: Request) {
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [item] = await tx.insert(emergingRisk).values({
      orgId: ctx.orgId,
