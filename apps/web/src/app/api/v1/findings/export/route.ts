...
// delegates to the finding entity registry entry added in PR #144.

export const GET = withErrorHandler(async function GET(req: Request) {
  const ctx = await withAuth();
  if (ctx instanceof Response) return ctx;

  // findings live in the ICS module per the controls/findings entity
  // graph; the requireModule key matches the existing /findings GET.
  ...
  const result = await exportEntities("finding", format, filters, ctx.orgId);
  ...
  try {
    await db.insert(dataExportLog).values({
      ...
      entityType: "finding",
      description: `Findings export (${format.toUpperCase()}, ${result.rowCount} records)`,
