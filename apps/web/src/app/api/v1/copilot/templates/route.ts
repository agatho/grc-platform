...
export async function GET(req: Request) {
  ...
  const query = promptTemplateQuerySchema.safeParse(
  ...
  const conditions = [
    or(
      ...
      isNull(copilotPromptTemplate.orgId),
    ),
  ];

  if (query.data.category)
    conditions.push(eq(copilotPromptTemplate.category, query.data.category));
  if (query.data.moduleKey)
    conditions.push(eq(copilotPromptTemplate.moduleKey, query.data.moduleKey));
  if (query.data.isActive !== undefined)
    conditions.push(eq(copilotPromptTemplate.isActive, query.data.isActive));
