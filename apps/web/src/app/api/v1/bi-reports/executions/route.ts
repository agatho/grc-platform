...
import { triggerBiReportExecutionSchema, listBiExecutionsQuerySchema } from "@grc/shared";
...
export async function GET(req: Request) {
  ...
  const query = listBiExecutionsQuerySchema.parse(Object.fromEntries(url.searchParams));
  const conditions = [eq(biReportExecution.orgId, ctx.orgId)];
  if (query.reportId) conditions.push(eq(biReportExecution.reportId, query.reportId));
  if (query.status) conditions.push(eq(biReportExecution.status, query.status));

  const offset = (query.page - 1) * query.limit;
  const [rows, [{ total }]] = await Promise.all([
    db.select().from(biReportExecution).where(and(...conditions))
      .orderBy(desc(biReportExecution.createdAt)).limit(query.limit).offset(offset),
    db.select({ total: sql<number>`count(*)::int` }).from(biReportExecution).where(and(...conditions)),
  ]);
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(biReportExecution).values({
      orgId: ctx.orgId,
