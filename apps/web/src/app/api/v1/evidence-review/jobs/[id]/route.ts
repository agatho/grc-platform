...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [job] = await db.select().from(evidenceReviewJob)
    .where(and(eq(evidenceReviewJob.id, id), eq(evidenceReviewJob.orgId, ctx.orgId)));
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(evidenceReviewJob)
  ...
  if (!result) return Response.json({ error: "Not found or not running" }, { status: 404 });
