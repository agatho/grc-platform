...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const query = evidenceReviewResultQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  if (!query.success) {
  ...
  if (classification) conditions.push(eq(evidenceReviewResult.classification, classification));
  if (controlId) conditions.push(eq(evidenceReviewResult.controlId, controlId));
  if (minConfidence !== undefined) conditions.push(gte(evidenceReviewResult.confidenceScore, String(minConfidence)));

  const [results, countResult] = await Promise.all([
    db.select().from(evidenceReviewResult)
      .where(and(...conditions))
      .orderBy(desc(evidenceReviewResult.reviewedAt))
      .limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(evidenceReviewResult)
      .where(and(...conditions)),
  ]);
