...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const query = evidenceReviewGapQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  if (!query.success) {
  ...
  const [gaps, countResult] = await Promise.all([
    db.select().from(evidenceReviewGap)
      .where(and(...conditions))
      .orderBy(desc(evidenceReviewGap.createdAt))
      .limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(evidenceReviewGap)
      .where(and(...conditions)),
  ]);
