...
import { createProtectionCaseSchema, createProtectionEventSchema } from "@grc/shared";
...
export async function GET(req: Request) {
  ...
  const rows = await db.select().from(wbProtectionCase)
...
export async function POST(req: Request) {
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [row] = await tx.insert(wbProtectionCase).values({
      orgId: ctx.orgId,
