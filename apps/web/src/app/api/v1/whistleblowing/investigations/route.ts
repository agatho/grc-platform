...
import {
  ...
  advanceInvestigationPhaseSchema,
...
export async function PATCH(req: Request) {
  ...
  const [current] = await db
    ...
    .where(
      and(
        eq(wbInvestigation.id, investigationId),
        eq(wbInvestigation.orgId, ctx.orgId),
      ),
    );
  if (!current)
  ...
  if (!isValidInvestigationTransition(current.phase, body.data.newPhase)) {
    return Response.json(
      {
        error: `Invalid transition from ${current.phase} to ${body.data.newPhase}`,
      },
      { status: 400 },
    );
  }
  ...
  const updated = await withAuditContext(ctx, async (tx) => {
    const [row] = await tx
      .update(wbInvestigation)
      .set({
        phase: body.data.newPhase,
        ...phaseTimestamps[body.data.newPhase],
        updatedAt: new Date(),
      })
