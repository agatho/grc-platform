...
export async function POST(req: Request) {
  ...
  const estimatedTotalCost = gapsWithCost.reduce((s, g) => s + g.estimatedCost, 0);
...
): Array<{ requirement: string; missingControl: string; effort: "S" | "M" | "L" | "XL" }> {
  // Deterministic gap calculation based on scenario type
  switch (scenarioType) {
    case "add_requirement":
      return [
        { requirement: "New data classification requirement", missingControl: "Data classification policy", effort: "M" },
        { requirement: "New access review requirement", missingControl: "Quarterly access review process", effort: "L" },
      ];
    case "tighten":
      return [
        { requirement: "Enhanced encryption standard", missingControl: "AES-256 for data at rest", effort: "L" },
      ];
    case "shorten_deadline":
      return [
        { requirement: "Accelerated reporting timeline", missingControl: "Automated incident reporting", effort: "XL" },
      ];
    case "add_reporting":
      return [
        { requirement: "Quarterly compliance report", missingControl: "Automated report generation", effort: "M" },
        { requirement: "Board notification template", missingControl: "Executive summary template", effort: "S" },
      ];
    default:
      return [];
  }
