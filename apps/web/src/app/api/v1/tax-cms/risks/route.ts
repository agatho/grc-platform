...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(taxRisk).values({ ...body.data, orgId: ctx.orgId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = taxRiskQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  if (!query.success) return Response.json({ error: "Invalid query", details: query.error.flatten() }, { status: 422 });
  const { page, limit } = query.data;
  const offset = (page - 1) * limit;
  const [rows, countResult] = await Promise.all([
    db.select().from(taxRisk).where(eq(taxRisk.orgId, ctx.orgId)).orderBy(desc(taxRisk.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(taxRisk).where(eq(taxRisk.orgId, ctx.orgId)),
  ]);
