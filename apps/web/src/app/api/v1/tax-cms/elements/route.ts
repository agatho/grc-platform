...
import { createTaxCmsElementSchema, taxCmsElementQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(taxCmsElement).values({ ...body.data, orgId: ctx.orgId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = taxCmsElementQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  ...
  const [rows, countResult] = await Promise.all([
    db.select().from(taxCmsElement).where(and(...conditions)).orderBy(desc(taxCmsElement.elementNumber)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(taxCmsElement).where(and(...conditions)),
  ]);
