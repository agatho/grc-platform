...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(taxAuditPrep).values({ ...body.data, orgId: ctx.orgId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = taxAuditPrepQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  if (!query.success) return Response.json({ error: "Invalid query", details: query.error.flatten() }, { status: 422 });
  const { page, limit } = query.data;
  const offset = (page - 1) * limit;
  const [rows, countResult] = await Promise.all([
    db.select().from(taxAuditPrep).where(eq(taxAuditPrep.orgId, ctx.orgId)).orderBy(desc(taxAuditPrep.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(taxAuditPrep).where(eq(taxAuditPrep.orgId, ctx.orgId)),
  ]);
