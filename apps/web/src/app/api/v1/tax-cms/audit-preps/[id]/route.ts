...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.select().from(taxAuditPrep).where(and(eq(taxAuditPrep.id, id), eq(taxAuditPrep.orgId, ctx.orgId)));
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(taxAuditPrep).set({ ...body.data, updatedAt: new Date() }).where(and(eq(taxAuditPrep.id, id), eq(taxAuditPrep.orgId, ctx.orgId))).returning();
    return updated;
  });
