import { db, academyCourse, academyEnrollment, academyCertificate } from "@grc/db";
...
export async function GET(req: Request) {
  ...
  const query = academyDashboardQuerySchema.parse(Object.fromEntries(url.searchParams));
  ...
  }).from(academyCourse).where(eq(academyCourse.orgId, ctx.orgId));
  ...
  }).from(academyEnrollment).where(eq(academyEnrollment.orgId, ctx.orgId));

  const [certStats] = await db.select({
    totalCertificates: sql<number>`count(*)::int`,
  }).from(academyCertificate).where(eq(academyCertificate.orgId, ctx.orgId));

  const completionRate = enrollmentStats.totalEnrollments > 0
    ? Math.round((enrollmentStats.completedEnrollments / enrollmentStats.totalEnrollments) * 100) : 0;
