...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [anomaly] = await db.select().from(riskAnomalyDetection)
    .where(and(eq(riskAnomalyDetection.id, id), eq(riskAnomalyDetection.orgId, ctx.orgId)));
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const isResolved = body.data.status === "resolved" || body.data.status === "false_positive";

  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(riskAnomalyDetection)
      .set({
