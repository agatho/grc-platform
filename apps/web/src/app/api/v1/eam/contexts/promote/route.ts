...
export async function POST(req: Request) {
  ...
  const context = await db.select().from(eamContext)
    .where(and(eq(eamContext.id, contextId), eq(eamContext.orgId, ctx.orgId)))
    .limit(1);

  if (!context.length) return Response.json({ error: "Context not found" }, { status: 404 });
  if (context[0].isDefault) return Response.json({ error: "Context is already default" }, { status: 400 });

  // In a transaction: deactivate current default, promote new one
  // Remove current default
  await db.update(eamContext)
    .set({ isDefault: false, status: "archived", updatedAt: new Date() })
    .where(and(eq(eamContext.orgId, ctx.orgId), eq(eamContext.isDefault, true)));

  // Promote new default
  const promoted = await db.update(eamContext)
