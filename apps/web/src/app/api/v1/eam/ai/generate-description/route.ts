...
export async function POST(req: Request) {
  ...
  const element = await db.select().from(architectureElement)
    .where(and(eq(architectureElement.id, parsed.data.entityId), eq(architectureElement.orgId, ctx.orgId)))
    .limit(1);

  if (!element.length) return Response.json({ error: "Entity not found" }, { status: 404 });
