...
export async function PUT(req: Request) {
  ...
  if (parsed.data.examinerId !== undefined) updateData.examinerId = parsed.data.examinerId;
  if (parsed.data.responsibleId !== undefined) updateData.responsibleId = parsed.data.responsibleId;

  const updated = await db.update(architectureElement)
    .set(updateData)
    .where(and(eq(architectureElement.id, elementId), eq(architectureElement.orgId, ctx.orgId)))
    .returning();

  if (!updated.length) return Response.json({ error: "Element not found" }, { status: 404 });
