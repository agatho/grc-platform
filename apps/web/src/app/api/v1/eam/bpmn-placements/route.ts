...
export async function GET(req: Request) {
  const ctx = await withAuth("admin", "risk_manager", "process_owner", "viewer");
  ...
  if (!processVersionId) return Response.json({ error: "processVersionId required" }, { status: 400 });

  const placements = await db.select().from(eamBpmnElementPlacement)
    .where(and(
...
export async function POST(req: Request) {
  ...
  const created = await db.insert(eamBpmnElementPlacement).values({
    ...parsed.data,
    orgId: ctx.orgId,
  }).returning();
...
export async function DELETE(req: Request) {
  ...
  if (!placementId) return Response.json({ error: "id required" }, { status: 400 });

  const deleted = await db.delete(eamBpmnElementPlacement)
    .where(and(eq(eamBpmnElementPlacement.id, placementId), eq(eamBpmnElementPlacement.orgId, ctx.orgId)))
    .returning();

  if (!deleted.length) return Response.json({ error: "Placement not found" }, { status: 404 });
