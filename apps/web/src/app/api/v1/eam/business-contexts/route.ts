...
import { createBusinessContextSchema, updateBusinessContextSchema } from "@grc/shared";
...
export async function GET(req: Request) {
  ...
  const contexts = await db.select().from(eamBusinessContext)
...
export async function POST(req: Request) {
  ...
  const created = await db.insert(eamBusinessContext).values({
    ...parsed.data,
    orgId: ctx.orgId,
  }).returning();
...
export async function PUT(req: Request) {
  ...
  const updated = await db.update(eamBusinessContext)
    .set({ ...parsed.data, updatedAt: new Date() })
    .where(and(eq(eamBusinessContext.id, contextId), eq(eamBusinessContext.orgId, ctx.orgId)))
    .returning();

  if (!updated.length) return Response.json({ error: "Business context not found" }, { status: 404 });
...
export async function DELETE(req: Request) {
  ...
  const deleted = await db.delete(eamBusinessContext)
    .where(and(eq(eamBusinessContext.id, contextId), eq(eamBusinessContext.orgId, ctx.orgId)))
    .returning();

  if (!deleted.length) return Response.json({ error: "Business context not found" }, { status: 404 });
