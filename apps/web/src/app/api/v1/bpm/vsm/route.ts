...
import { createVsmSchema, computeVsmMetrics, computeWasteAnalysis } from "@grc/shared";
...
export async function GET(req: Request) {
  ...
  const rows = await db.select().from(valueStreamMap)
...
export async function POST(req: Request) {
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [row] = await tx.insert(valueStreamMap).values({
      orgId: ctx.orgId,
