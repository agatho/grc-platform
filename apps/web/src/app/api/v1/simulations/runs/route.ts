...
export async function GET(req: Request) {
  ...
  const rows = await db.select().from(simulationRun)
    .where(and(eq(simulationRun.scenarioId, scenarioId), eq(simulationRun.orgId, ctx.orgId)))
...
export async function POST(req: Request) {
  ...
  const [{ count }] = await db.select({ count: sql<number>`count(*)::int` })
    .from(simulationRun).where(eq(simulationRun.scenarioId, body.scenarioId));
