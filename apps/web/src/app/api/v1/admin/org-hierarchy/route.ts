...
export async function GET(req: Request) {
  ...
  const orgs = await db
    ...
    .from(organization)
    .orderBy(organization.hierarchyLevel);

  // Build tree structure
  const orgMap = new Map(
    orgs.map((o) => [o.id, { ...o, children: [] as typeof orgs }]),
  );
...
export async function PUT(req: Request) {
  ...
  if (parentOrgId) {
    const [parent] = await db
      .select({ hierarchyLevel: organization.hierarchyLevel })
      .from(organization)
      .where(eq(organization.id, parentOrgId));
    if (!parent) {
      return Response.json(
    ...
    if ((parent.hierarchyLevel ?? 0) >= 3) {
      return Response.json(
        { error: "Maximum hierarchy depth of 4 exceeded" },
        { status: 422 },
      );
    }
    // Prevent cycles
  ...
  const updated = await withAuditContext(ctx, async (tx) => {
    const level = parentOrgId
      ? ((
          await tx
            .select({ l: organization.hierarchyLevel })
            .from(organization)
            .where(eq(organization.id, parentOrgId))
        )[0]?.l ?? 0) + 1
      : 0;
