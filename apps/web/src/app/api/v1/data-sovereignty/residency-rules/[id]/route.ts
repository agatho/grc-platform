...
export async function GET(
  ...
  const [row] = await db
    ...
    .where(
      and(eq(dataResidencyRule.id, id), eq(dataResidencyRule.orgId, ctx.orgId)),
    );
...
export async function PATCH(
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx
      .update(dataResidencyRule)
      .set({ ...body, updatedAt: new Date() })
      .where(
        and(
          eq(dataResidencyRule.id, id),
          eq(dataResidencyRule.orgId, ctx.orgId),
        ),
      )
      .returning();
    return updated;
