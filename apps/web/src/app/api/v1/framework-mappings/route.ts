...
export async function POST(req: Request) {
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [row] = await tx.insert(frameworkMapping).values({
      ...body.data,
...
export async function GET(req: Request) {
  ...
  if (sourceFramework) conditions.push(eq(frameworkMapping.sourceFramework, sourceFramework));
  const targetFramework = searchParams.get("targetFramework");
  if (targetFramework) conditions.push(eq(frameworkMapping.targetFramework, targetFramework));
  ...
  if (isVerified === "true") conditions.push(eq(frameworkMapping.isVerified, true));
  if (isVerified === "false") conditions.push(eq(frameworkMapping.isVerified, false));
  const minConfidence = searchParams.get("minConfidence");
  if (minConfidence) conditions.push(gte(frameworkMapping.confidence, minConfidence));

  const where = conditions.length > 0 ? and(...conditions) : undefined;

  const [items, [{ value: total }]] = await Promise.all([
    db.select().from(frameworkMapping).where(where).orderBy(desc(frameworkMapping.confidence)).limit(limit).offset(offset),
