...
import { createRegulatorySourceSchema, regulatorySourceQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(regulatorySource)
...
export async function GET(req: Request) {
  ...
  const query = regulatorySourceQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  if (!query.success) {
  ...
  const conditions = [or(eq(regulatorySource.orgId, ctx.orgId), isNull(regulatorySource.orgId))];
  if (jurisdiction) conditions.push(eq(regulatorySource.jurisdiction, jurisdiction));
  if (sourceType) conditions.push(eq(regulatorySource.sourceType, sourceType));
  if (isActive !== undefined) conditions.push(eq(regulatorySource.isActive, isActive));

  const [sources, countResult] = await Promise.all([
    db.select().from(regulatorySource)
      .where(and(...conditions))
      .orderBy(desc(regulatorySource.createdAt))
      .limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(regulatorySource)
      .where(and(...conditions)),
  ]);
