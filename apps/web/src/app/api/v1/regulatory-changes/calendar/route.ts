...
import { createCalendarEventSchema, calendarEventQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(regulatoryCalendarEvent)
...
export async function GET(req: Request) {
  ...
  const query = calendarEventQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  if (!query.success) {
    return Response.json({ error: "Invalid query", details: query.error.flatten() }, { status: 422 });
  }

  const { page, limit, from, to, priority, eventType, isCompleted } = query.data;
  ...
  if (eventType) conditions.push(eq(regulatoryCalendarEvent.eventType, eventType));
  if (isCompleted !== undefined) conditions.push(eq(regulatoryCalendarEvent.isCompleted, isCompleted));

  const [events, countResult] = await Promise.all([
    db.select().from(regulatoryCalendarEvent)
      .where(and(...conditions))
      .orderBy(regulatoryCalendarEvent.eventDate)
      .limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(regulatoryCalendarEvent)
      .where(and(...conditions)),
  ]);
