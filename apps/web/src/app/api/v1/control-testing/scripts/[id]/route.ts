...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [script] = await db.select().from(controlTestScript)
    .where(and(eq(controlTestScript.id, id), eq(controlTestScript.orgId, ctx.orgId)));
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(controlTestScript)
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const result = await withAuditContext(ctx, async (tx) => {
    const [deleted] = await tx.delete(controlTestScript)
      .where(and(eq(controlTestScript.id, id), eq(controlTestScript.orgId, ctx.orgId)))
      .returning();
    return deleted;
  });
