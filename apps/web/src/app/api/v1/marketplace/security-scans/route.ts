...
export async function GET(req: Request) {
  ...
  const rows = await db.select().from(marketplaceSecurityScan)
    .where(and(eq(marketplaceSecurityScan.versionId, versionId), eq(marketplaceSecurityScan.orgId, ctx.orgId)))
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(marketplaceSecurityScan).values({
      orgId: ctx.orgId,
