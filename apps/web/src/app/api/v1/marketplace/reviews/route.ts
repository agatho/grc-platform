...
export async function GET(req: Request) {
  ...
  const rows = await db.select().from(marketplaceReview)
    .where(and(eq(marketplaceReview.listingId, listingId), eq(marketplaceReview.orgId, ctx.orgId)))
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(marketplaceReview).values({
    ...
    }).from(marketplaceReview).where(eq(marketplaceReview.listingId, body.listingId));
