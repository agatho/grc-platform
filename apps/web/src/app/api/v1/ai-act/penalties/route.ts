...
export async function POST(req: Request) {
  ...
  const { authority, penalty_type, fine_amount, fine_currency, article_reference, description, ai_system_id, appeal_status } = body;
  if (!authority || !penalty_type) {
    return Response.json({ error: "authority and penalty_type are required" }, { status: 422 });
  }
