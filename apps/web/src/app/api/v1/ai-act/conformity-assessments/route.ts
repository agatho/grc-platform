...
import {
  createAiConformityAssessmentSchema,
  aiConformityQuerySchema,
} from "@grc/shared";
...
export async function GET(req: Request) {
  ...
  const query = aiConformityQuerySchema.safeParse(
  ...
  if (aiSystemId)
    conditions.push(eq(aiConformityAssessment.aiSystemId, aiSystemId));
  if (overallResult)
    conditions.push(eq(aiConformityAssessment.overallResult, overallResult));
