...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(aiTransparencyEntry).values({ ...body.data, orgId: ctx.orgId, publishedBy: ctx.userId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const [rows, countResult] = await Promise.all([
    db.select().from(aiTransparencyEntry).where(eq(aiTransparencyEntry.orgId, ctx.orgId)).orderBy(desc(aiTransparencyEntry.createdAt)).limit(limit).offset(offset),
