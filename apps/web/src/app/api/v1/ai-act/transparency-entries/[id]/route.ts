...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.select().from(aiTransparencyEntry).where(and(eq(aiTransparencyEntry.id, id), eq(aiTransparencyEntry.orgId, ctx.orgId)));
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(aiTransparencyEntry).set(updateData).where(and(eq(aiTransparencyEntry.id, id), eq(aiTransparencyEntry.orgId, ctx.orgId))).returning();
    return updated;
  });
