...
export async function GET(req: Request) {
  ...
  const [totalSystems, highRisk, unacceptable, pendingAssessments, oversightCount, transparencyCount, friasPending, docsDue] = await Promise.all([
    ...
    db.execute(sql`SELECT count(*)::int AS count FROM v_ai_documentation_status WHERE org_id = ${ctx.orgId} AND (review_overdue = true OR expiry_approaching = true)`),
  ]);
