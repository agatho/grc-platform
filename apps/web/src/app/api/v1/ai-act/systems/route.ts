...
export async function GET(req: Request) {
  ...
  const { page, limit, riskClassification, status, providerOrDeployer } =
    query.data;
  ...
  if (riskClassification)
    conditions.push(eq(aiSystem.riskClassification, riskClassification));
  if (status) conditions.push(eq(aiSystem.status, status));
  if (providerOrDeployer)
    conditions.push(eq(aiSystem.providerOrDeployer, providerOrDeployer));
