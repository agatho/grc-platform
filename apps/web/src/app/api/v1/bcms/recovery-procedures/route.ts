...
export async function GET(req: Request) {
  ...
  const [items, [{ value: total }]] = await Promise.all([
    db.select().from(recoveryProcedure).where(where)
      .orderBy(desc(recoveryProcedure.updatedAt)).limit(limit).offset(offset),
...
export async function POST(req: Request) {
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [procedure] = await tx.insert(recoveryProcedure).values({
      orgId: ctx.orgId,
    ...
    for (let i = 0; i < steps.length; i++) {
      const step = steps[i];
      const [inserted] = await tx.insert(recoveryProcedureStep).values({
        ...
        dependsOnStepId: step.dependsOnStepIndex !== undefined ? stepIdMap.get(step.dependsOnStepIndex) : undefined,
      }).returning();
