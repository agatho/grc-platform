...
export async function GET(
  ...
  const [tree] = await db
    ...
    .where(
      and(
        eq(crisisContactTree.id, treeId),
        eq(crisisContactTree.orgId, ctx.orgId),
      ),
    );
  if (!tree)
    return Response.json({ error: "Contact tree not found" }, { status: 404 });

  const nodes = await db
    .select()
    .from(crisisContactNode)
...
export async function POST(
  ...
  const { treeId } = await params;
  const body = createContactNodeSchema.safeParse({
    ...(await req.json()),
    treeId,
  });
  if (!body.success) {
  ...
  if (body.data.parentNodeId) {
    ...
    if (!parent || parent.treeId !== treeId) {
      return Response.json(
        { error: "Parent node not found in this tree" },
        { status: 422 },
      );
    }
  }
