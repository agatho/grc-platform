...
export async function GET(req: Request) {
  ...
  const [items, [{ value: total }]] = await Promise.all([
    db.select().from(crisisContactTree).where(where)
      .orderBy(desc(crisisContactTree.createdAt)).limit(limit).offset(offset),
...
export async function POST(req: Request) {
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [tree] = await tx.insert(crisisContactTree).values({
      orgId: ctx.orgId,
