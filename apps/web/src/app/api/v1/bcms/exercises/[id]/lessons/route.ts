...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const lessons = await db.select().from(bcExerciseLesson)
    .where(and(eq(bcExerciseLesson.exerciseId, id), eq(bcExerciseLesson.orgId, ctx.orgId)));
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [exercise] = await db.select().from(bcExercise)
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [lesson] = await tx.insert(bcExerciseLesson).values({
      exerciseId: id,
      orgId: ctx.orgId,
      ...body.data,
    }).returning();
