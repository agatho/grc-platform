...
import { createCertEvidencePackageSchema, certEvidenceQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(certEvidencePackage).values({ ...body.data, orgId: ctx.orgId, generatedBy: ctx.userId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = certEvidenceQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  ...
  if (assessmentId) conditions.push(eq(certEvidencePackage.assessmentId, assessmentId));
  if (status) conditions.push(eq(certEvidencePackage.status, status));

  const [rows, countResult] = await Promise.all([
    db.select().from(certEvidencePackage).where(and(...conditions)).orderBy(desc(certEvidencePackage.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(certEvidencePackage).where(and(...conditions)),
  ]);
