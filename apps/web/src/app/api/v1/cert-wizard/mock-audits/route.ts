...
import { createCertMockAuditSchema, certMockAuditQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(certMockAudit).values({ ...body.data, orgId: ctx.orgId, auditorId: ctx.userId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = certMockAuditQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  ...
  const [rows, countResult] = await Promise.all([
    db.select().from(certMockAudit).where(and(...conditions)).orderBy(desc(certMockAudit.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(certMockAudit).where(and(...conditions)),
  ]);
