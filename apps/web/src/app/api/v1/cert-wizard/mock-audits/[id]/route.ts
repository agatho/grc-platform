...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.select().from(certMockAudit).where(and(eq(certMockAudit.id, id), eq(certMockAudit.orgId, ctx.orgId)));
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.data.responses) {
    ...
    const scores = body.data.responses.filter((r) => r.aiScore !== undefined).map((r) => r.aiScore!);
    if (scores.length > 0) updateData.overallScore = (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(2);
  }
  if (body.data.status === "in_progress" && !updateData.startedAt) updateData.startedAt = new Date();
  if (body.data.status === "completed") updateData.completedAt = new Date();

  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(certMockAudit).set(updateData).where(and(eq(certMockAudit.id, id), eq(certMockAudit.orgId, ctx.orgId))).returning();
    return updated;
  });
