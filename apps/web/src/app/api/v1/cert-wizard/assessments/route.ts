...
import { createCertReadinessSchema, certReadinessQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(certReadinessAssessment).values({ ...body.data, orgId: ctx.orgId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = certReadinessQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  ...
  if (framework) conditions.push(eq(certReadinessAssessment.framework, framework));
  if (status) conditions.push(eq(certReadinessAssessment.status, status));

  const [rows, countResult] = await Promise.all([
    db.select().from(certReadinessAssessment).where(and(...conditions)).orderBy(desc(certReadinessAssessment.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(certReadinessAssessment).where(and(...conditions)),
  ]);
