...
export async function GET(
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(certReadinessAssessment.id, id),
        eq(certReadinessAssessment.orgId, ctx.orgId),
      ),
    );
...
export async function PATCH(
  ...
  if (body.data.controlDetails) {
    ...
    updateData.implementedControls = details.filter(
      (d) => d.status === "implemented",
    ).length;
    updateData.partialControls = details.filter(
      (d) => d.status === "partial",
    ).length;
    updateData.notImplemented = details.filter(
      (d) => d.status === "not_implemented",
    ).length;
    updateData.notApplicable = details.filter(
    ...
    updateData.readinessScore =
      applicable > 0
        ? (
            (((updateData.implementedControls as number) +
              (updateData.partialControls as number) * 0.5) /
              applicable) *
            100
          ).toFixed(2)
        : "0";
...
export async function DELETE(
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [deleted] = await tx
      .delete(certReadinessAssessment)
      .where(
        and(
          eq(certReadinessAssessment.id, id),
          eq(certReadinessAssessment.orgId, ctx.orgId),
