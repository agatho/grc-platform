...
export async function POST(req: Request) {
  ...
  const created = await withAuditContext(ctx, async (tx) => {
    const [row] = await tx.insert(identityConnectorConfig).values({ orgId: ctx.orgId, ...body.data }).returning();
    return row;
  });
...
export async function GET(req: Request) {
  ...
  if (provider) conditions.push(eq(identityConnectorConfig.identityProvider, provider));

  const where = and(...conditions);

  const [items, [{ value: total }]] = await Promise.all([
    db.select().from(identityConnectorConfig).where(where).orderBy(desc(identityConnectorConfig.createdAt)).limit(limit).offset(offset),
