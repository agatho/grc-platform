...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.select().from(portalQuestionnaireResponse)
    .where(and(eq(portalQuestionnaireResponse.id, id), eq(portalQuestionnaireResponse.orgId, ctx.orgId)));
