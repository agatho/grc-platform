...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.select().from(portalConfig)
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(portalConfig).set({ ...body, updatedAt: new Date() })
      .where(and(eq(portalConfig.id, id), eq(portalConfig.orgId, ctx.orgId))).returning();
    return updated;
  });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const result = await withAuditContext(ctx, async (tx) => {
    const [deleted] = await tx.delete(portalConfig)
      .where(and(eq(portalConfig.id, id), eq(portalConfig.orgId, ctx.orgId))).returning();
    return deleted;
  });
  if (!result) return Response.json({ error: "Not found" }, { status: 404 });
