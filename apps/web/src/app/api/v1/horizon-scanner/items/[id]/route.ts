...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.select().from(horizonScanItem).where(and(eq(horizonScanItem.id, id), eq(horizonScanItem.orgId, ctx.orgId)));
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(horizonScanItem).set({ ...body.data, reviewedBy: ctx.userId, reviewedAt: new Date(), updatedAt: new Date() }).where(and(eq(horizonScanItem.id, id), eq(horizonScanItem.orgId, ctx.orgId))).returning();
    return updated;
  });
