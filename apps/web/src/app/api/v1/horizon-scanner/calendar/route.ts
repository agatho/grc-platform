...
import { createHorizonCalendarEventSchema, horizonCalendarQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(horizonCalendarEvent).values({ ...body.data, orgId: ctx.orgId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = horizonCalendarQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  ...
  if (isCompleted !== undefined) conditions.push(eq(horizonCalendarEvent.isCompleted, isCompleted));

  const [rows, countResult] = await Promise.all([
    db.select().from(horizonCalendarEvent).where(and(...conditions)).orderBy(desc(horizonCalendarEvent.eventDate)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(horizonCalendarEvent).where(and(...conditions)),
  ]);
