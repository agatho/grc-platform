...
export async function POST(req: Request) {
  ...
  const [model] = await db.select().from(maturityModel)
    .where(and(eq(maturityModel.orgId, ctx.orgId), eq(maturityModel.moduleKey, body.moduleKey)));
  if (!model) return Response.json({ error: "No maturity model found for this module. Please create one first." }, { status: 404 });

  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(benchmarkSubmission).values({
      orgId: ctx.orgId,
