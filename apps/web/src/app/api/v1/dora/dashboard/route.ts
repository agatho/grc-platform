...
export async function GET(req: Request) {
  ...
  const [
    ...
    db
      ...
      .from(doraIctRisk)
      .where(
        and(eq(doraIctRisk.orgId, ctx.orgId), isNull(doraIctRisk.deletedAt)),
      ),
    db
      .select({ count: sql<number>`count(*)` })
      .from(doraIctRisk)
      .where(
        and(
          ...
          eq(doraIctRisk.riskLevel, "critical"),
    ...
    db
      ...
      .where(
        and(
          eq(doraIctIncident.orgId, ctx.orgId),
    ...
    db
      ...
      .where(
        and(
          eq(doraIctIncident.orgId, ctx.orgId),
          eq(doraIctIncident.classification, "major"),
          ne(doraIctIncident.status, "closed"),
        ),
      ),
    db
      ...
      .where(eq(doraIctProvider.orgId, ctx.orgId)),
    db
      .select({ count: sql<number>`count(*)` })
      .from(doraIctProvider)
      .where(
        and(
          eq(doraIctProvider.orgId, ctx.orgId),
          eq(doraIctProvider.criticality, "critical"),
        ),
      ),
    db
      .select({ count: sql<number>`count(*)` })
      ...
      .where(
        and(
          eq(doraIctIncident.orgId, ctx.orgId),
          sql`initial_report_sent IS NULL AND initial_report_due < now()`,
        ),
      ),
    db
      .select({ count: sql<number>`count(*)` })
      ...
      .where(
        and(
          eq(doraNis2CrossRef.orgId, ctx.orgId),
          eq(doraNis2CrossRef.complianceStatus, "compliant"),
        ),
      ),
  ]);

  const totalNis2 = await db
    .select({ count: sql<number>`count(*)` })
    .from(doraNis2CrossRef)
    .where(eq(doraNis2CrossRef.orgId, ctx.orgId));
  ...
  return Response.json({
    data: {
      ...
      nis2ComplianceRate:
        Number(totalNis2[0]?.count) > 0
          ? Math.round(
              (Number(nis2Stats[0]?.count ?? 0) / Number(totalNis2[0]?.count)) *
                100,
            )
          : 0,
      upcomingTlptTests: [],
