...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.select().from(doraInformationSharing).where(and(eq(doraInformationSharing.id, id), eq(doraInformationSharing.orgId, ctx.orgId)));
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [updated] = await tx.update(doraInformationSharing).set(updateData).where(and(eq(doraInformationSharing.id, id), eq(doraInformationSharing.orgId, ctx.orgId))).returning();
    return updated;
  });
