...
import { createDoraIctProviderSchema, doraIctProviderQuerySchema } from "@grc/shared";
...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(doraIctProvider).values({ ...body.data, orgId: ctx.orgId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = doraIctProviderQuerySchema.safeParse(Object.fromEntries(url.searchParams));
  ...
  if (criticality) conditions.push(eq(doraIctProvider.criticality, criticality));
  if (status) conditions.push(eq(doraIctProvider.status, status));
  if (complianceStatus) conditions.push(eq(doraIctProvider.complianceStatus, complianceStatus));

  const [rows, countResult] = await Promise.all([
    db.select().from(doraIctProvider).where(and(...conditions)).orderBy(desc(doraIctProvider.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(doraIctProvider).where(and(...conditions)),
  ]);
