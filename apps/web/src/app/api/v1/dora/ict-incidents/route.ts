  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx
      ...
      .values({
        ...
        intermediateReportDue: new Date(
          detectedAt.getTime() + 72 * 60 * 60 * 1000,
        ),
        finalReportDue: new Date(
          detectedAt.getTime() + 30 * 24 * 60 * 60 * 1000,
        ),
      })
      .returning();
    return created;
...
export async function GET(req: Request) {
  ...
  const query = doraIctIncidentQuerySchema.safeParse(
  ...
  if (classification)
    conditions.push(eq(doraIctIncident.classification, classification));
  ...
  const [rows, countResult] = await Promise.all([
    db
      ...
      .orderBy(desc(doraIctIncident.detectedAt))
      .limit(limit)
      .offset(offset),
    db
