...
import { createDoraTlptPlanSchema, doraTlptQuerySchema } from "@grc/shared";
...
export async function GET(req: Request) {
  ...
  const conditions = [eq(doraTlptPlan.orgId, ctx.orgId)];
  if (status) conditions.push(eq(doraTlptPlan.status, status));
