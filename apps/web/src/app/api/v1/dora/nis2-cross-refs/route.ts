...
export async function POST(req: Request) {
  ...
  const result = await withAuditContext(ctx, async (tx) => {
    const [created] = await tx.insert(doraNis2CrossRef).values({ ...body.data, orgId: ctx.orgId }).returning();
    return created;
  });
...
export async function GET(req: Request) {
  ...
  const query = doraNis2QuerySchema.safeParse(Object.fromEntries(url.searchParams));
  ...
  if (overlapType) conditions.push(eq(doraNis2CrossRef.overlapType, overlapType));
  if (complianceStatus) conditions.push(eq(doraNis2CrossRef.complianceStatus, complianceStatus));

  const [rows, countResult] = await Promise.all([
    db.select().from(doraNis2CrossRef).where(and(...conditions)).orderBy(desc(doraNis2CrossRef.createdAt)).limit(limit).offset(offset),
    db.select({ count: sql<number>`count(*)` }).from(doraNis2CrossRef).where(and(...conditions)),
  ]);
