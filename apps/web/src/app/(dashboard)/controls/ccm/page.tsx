...
function CCMDashboardInner() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{t("totalMonitored")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">
              {data?.totalMonitored ?? 0}
            </div>
          </CardContent>
        </Card>
        <Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("controlStatus")}</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            {data?.controls?.map((ctrl) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    variant={
                      ctrl.latestResult === "pass"
                        ? "default"
                        : ctrl.latestResult === "degraded"
                          ? "secondary"
                          : "destructive"
                    }
                  >
                  ...
                  <span className="text-sm text-muted-foreground">
                    {ctrl.latestScore}/100
                  </span>
                </div>
              </div>
            )) ?? (
              <p className="text-muted-foreground">
                {t("noMonitoredControls")}
              </p>
            )}
          </div>
        </CardContent>
