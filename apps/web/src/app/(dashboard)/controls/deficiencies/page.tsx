...
function DeficienciesInner() {
  ...
  const statusColumns = ["open", "in_progress", "remediated", "retesting", "closed"];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button><Plus className="mr-2 h-4 w-4" />{t("addDeficiency")}</Button>
      </div>

      {/* Kanban Board */}
      <div className="grid grid-cols-5 gap-4">
        {statusColumns.map((status) => {
          const columnItems = items.filter((i) => i.remediationStatus === status);
          return (
                ...
                <Card key={item.id} className="cursor-pointer hover:shadow-md transition-shadow">
                  <CardContent className="p-3 space-y-2">
                    ...
                    {item.remediationDeadline && (
                      <p className="text-xs text-muted-foreground">{t("deadline")}: {item.remediationDeadline}</p>
                    )}
                  </CardContent>
                </Card>
              ))}
