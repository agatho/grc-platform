  ...
  if (!data) {
    return (
      <p className="text-center text-gray-500 py-12">
        Nichtkonformität nicht gefunden.
      </p>
    );
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <Badge
                className={SEVERITY_COLORS[data.severity] ?? "bg-gray-100"}
              >
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <FieldRow
            label="Quelle"
            value={SOURCE_LABELS[data.source_type] ?? data.source_type}
          />
          <FieldRow
            label="Schweregrad"
            value={SEVERITY_LABELS[data.severity] ?? data.severity}
          />
          ...
          <div className="space-y-3">
            <div>
              ...
              <dd className="text-sm text-gray-900 mt-0.5">
                {ROOT_CAUSE_LABELS[data.root_cause_method ?? ""] ??
                  data.root_cause_method ??
                  "—"}
              </dd>
            </div>
            <div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6">
        ...
        {actions.length === 0 ? (
          <p className="text-sm text-gray-400 italic">
            Keine Korrekturmaßnahmen erfasst.
          </p>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        className={
                          CA_STATUS_COLORS[action.status] ?? "bg-gray-100"
                        }
                      >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <p
                        className={`font-medium ${caOverdue ? "text-red-600" : "text-gray-800"}`}
                      >
                    ...
                    <div className="rounded-md bg-green-50 border border-green-200 p-3 text-xs space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-green-800">
                          Verifiziert von {action.verified_by_name ?? "—"} am{" "}
                          {fmtDate(action.verified_at)}
                        </span>
                      </div>
                      {action.verification_result && (
                        <p className="text-green-700">
                          Ergebnis: {action.verification_result}
                        </p>
                      )}
                    ...
                    <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-xs space-y-1">
                      ...
                      {action.effectiveness_review_date && (
                        <p className="text-blue-700">
                          Datum: {fmtDate(action.effectiveness_review_date)}
                        </p>
                      )}
