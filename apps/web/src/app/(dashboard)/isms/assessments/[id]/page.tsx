    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {assessment.framework} | {assessment.periodStart} -{" "}
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between text-sm mb-2">
          ...
          <span className="text-gray-500">
            ...
            {assessment.completedEvaluations}/{assessment.totalEvaluations})
          </span>
        </div>
      ...
      {activeTab === "overview" && p && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            label={t("evaluation.effective")}
            value={p.controls?.effective ?? 0}
            color="text-green-600"
          />
          <StatCard
            label={t("evaluation.partiallyEffective")}
            value={p.controls?.partiallyEffective ?? 0}
            color="text-yellow-600"
          />
          <StatCard
            label={t("evaluation.ineffective")}
            value={p.controls?.ineffective ?? 0}
            color="text-red-600"
          />
          <StatCard
            label={t("evaluation.notEvaluated")}
            value={p.controls?.notEvaluated ?? 0}
            color="text-gray-400"
          />
          <StatCard
            label={t("maturity.avgCurrent")}
            value={p.maturity?.avgCurrent ?? 0}
            color="text-purple-600"
          />
          <StatCard
            label={t("maturity.avgTarget")}
            value={p.maturity?.avgTarget ?? 0}
            color="text-purple-400"
          />
          <StatCard
            label={t("maturity.gap")}
            value={p.maturity?.avgGap ?? 0}
            color="text-orange-600"
          />
          <StatCard
            label={t("riskEval.scenariosEvaluated")}
            value={`${p.risks?.completed ?? 0}/${p.risks?.total ?? 0}`}
            color="text-blue-600"
          />
        </div>
      )}
                ...
                evaluations.map((ev) => (
                  <tr key={ev.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-mono text-xs text-gray-600">
                      {ev.controlId.slice(0, 8)}...
                    </td>
                    <td className="px-4 py-3">
                      <Badge
                        variant="outline"
                        className={RESULT_COLORS[ev.result]}
                      >
                        {t(`evaluation.results.${ev.result}`)}
                      </Badge>
                    </td>
              ...
              {riskEvals.length === 0 ? (
                ...
                riskEvals.map((re) => (
                  <tr key={re.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-mono text-xs text-gray-600">
                      {re.riskScenarioId.slice(0, 8)}...
                    </td>
                    <td className="px-4 py-3 text-center">
                      {re.residualLikelihood ?? "-"}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {re.residualImpact ?? "-"}
                    </td>
