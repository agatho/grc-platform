    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{t("assessment.subtitle")}</p>
        </div>
      ...
      {/* Create Form */}
      {showCreate && (
        <CreateAssessmentForm onSubmit={handleCreate} onCancel={() => setShowCreate(false)} t={t} />
      )}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="min-w-0">
                  ...
                  {a.description && (
                    <p className="text-xs text-gray-500 mt-1 truncate">{a.description}</p>
                  )}
                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-400">
                    <span>{a.framework}</span>
                    {a.periodStart && a.periodEnd && (
                      <span>{a.periodStart} - {a.periodEnd}</span>
                    )}
                    <span>{a.completedEvaluations}/{a.totalEvaluations} {t("evaluation.evaluated")}</span>
                  </div>
                </div>
                <div className="shrink-0 ml-4 w-24">
                  ...
                  <p className="text-xs text-gray-500 text-center mt-1">{a.completionPercentage}%</p>
                </div>
              </div>
            </Link>
          ))}
...
function CreateAssessmentForm({
  ...
  onSubmit: (data: { name: string; description: string; scopeType: string; framework: string; periodStart: string; periodEnd: string }) => void;
  onCancel: () => void;
  ...
  return (
    <div className="rounded-lg border border-blue-200 bg-blue-50 p-6 space-y-4">
      <h3 className="text-sm font-semibold text-gray-900">{t("assessment.create")}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">{t("assessment.name")}</label>
          <input
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">{t("assessment.framework")}</label>
          <select
            className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
            value={framework}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">{t("assessment.scopeType")}</label>
          <select
            className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
            value={scopeType}
          ...
          >
            ...
            <option value="department">{t("assessment.scope.department")}</option>
            <option value="asset_group">{t("assessment.scope.assetGroup")}</option>
            <option value="custom">{t("assessment.scope.custom")}</option>
          </select>
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">{t("assessment.description")}</label>
          <input
            className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
            value={description}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">{t("assessment.periodStart")}</label>
          <input type="date" className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={periodStart} onChange={(e) => setPeriodStart(e.target.value)} />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">{t("assessment.periodEnd")}</label>
          <input type="date" className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={periodEnd} onChange={(e) => setPeriodEnd(e.target.value)} />
        </div>
      </div>
      <div className="flex justify-end gap-2">
        <Button variant="outline" size="sm" onClick={onCancel}>{t("actions.cancel")}</Button>
        <Button size="sm" onClick={() => onSubmit({ name, description, scopeType, framework, periodStart, periodEnd })} disabled={!name || !periodStart || !periodEnd}>
          {t("assessment.create")}
        </Button>
      </div>
    </div>
