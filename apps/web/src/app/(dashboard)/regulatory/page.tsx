  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/regulatory/sources">
            <Button variant="outline">{t("manageSources")}</Button>
          </Link>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t("totalChanges")}</p>
            <p className="text-2xl font-bold">{data.totalChanges}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t("newChanges")}</p>
          ...
          <CardContent>
            <div className="space-y-2">
              {data.recentChanges.map((change) => (
                <Link
                  key={change.id}
                  href={`/regulatory/changes?id=${change.id}`}
                  className="block"
                >
                  <div className="p-3 border rounded hover:bg-muted/50">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        className={
                          CLASSIFICATION_COLORS[change.classification] ?? ""
                        }
                      >
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {change.jurisdiction} - {change.changeType}
                    </p>
                  </div>
                </Link>
              ))}
