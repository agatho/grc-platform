...
export default function CreateCampaignPage() {
  ...
  const addDepartment = () => {
    if (
      ...
      !form.targetScope.departments.includes(departmentInput.trim())
    ) {
      updateForm({
        targetScope: {
          ...form.targetScope,
  ...
  const saveAndLaunch = async () => {
    ...
    try {
      ...
      const launchRes = await fetch(
        `/api/v1/rcsa/campaigns/${campaignId}/launch`,
        {
          method: "POST",
        },
      );
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 1:
        return (
          form.targetScope.departments.length > 0 ||
          form.targetScope.roles.length > 0
        );
      case 2:
        return true;
      case 3:
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  updateForm({
                    frequency: e.target.value as CampaignFormData["frequency"],
                  })
                }
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
              >
                <option value="quarterly">
                  {t("campaign.freq.quarterly")}
                </option>
                <option value="semi_annual">
                  {t("campaign.freq.semi_annual")}
                </option>
              ...
              <div className="flex flex-wrap gap-2">
                {["risk_owner", "control_owner"].map((role) => (
                  <Button
                    key={role}
                    variant={
                      form.targetScope.roles.includes(role)
                        ? "default"
                        : "outline"
                    }
                    size="sm"
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateForm({ cesWeight: Number(e.target.value) })
                }
                className="w-full"
              />
              <p className="text-xs text-gray-500 mt-1">
                {t("campaign.cesWeightHint")}
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <select
                ...
                onChange={(e) =>
                  updateForm({ reminderDaysBefore: Number(e.target.value) })
                }
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
              >
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">
                  {t(`campaign.freq.${form.frequency}`)}
                </p>
              </div>
              <div>
