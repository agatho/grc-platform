...
import { GripVertical, Plus, Save, Loader2, Eye } from "lucide-react";
...
export default function BiReportDesignerPage() {
  ...
  useEffect(() => {
    if (reportId) {
      fetch(`/api/v1/bi-reports/${reportId}`)
        .then((r) => r.json())
        .then((json) => {
          if (json.data) {
      ...
      fetch(`/api/v1/bi-reports/widgets?reportId=${reportId}`)
        .then((r) => r.json())
        .then((json) => {
          setWidgets(
            (json.data ?? []).map((w: Record<string, string>) => ({
              id: w.id,
              type: w.widgetType,
              title: w.title ?? "",
            })),
              ...
              {widgetTypes.map((wt) => (
                <Button
                  ...
                  onClick={() =>
                    setWidgets((prev) => [
                      ...prev,
                      { id: crypto.randomUUID(), type: wt, title: "" },
                    ])
                  }
                >
