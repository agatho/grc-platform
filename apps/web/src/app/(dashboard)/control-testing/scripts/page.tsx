...
export default function ControlTestScriptsPage() {
  ...
  const executeScript = async (id: string) => {
    await fetch(`/api/v1/control-testing/scripts/${id}/execute`, { method: "POST" });
    await fetchScripts();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button><Plus className="h-4 w-4 mr-2" />{t("scripts.create")}</Button>
          <Button variant="outline"><Sparkles className="h-4 w-4 mr-2" />{t("scripts.aiGenerate")}</Button>
        </div>
      </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                      ...
                      {script.aiGenerated && <Badge variant="secondary"><Sparkles className="h-3 w-3 mr-1" />{t("scripts.aiGenerated")}</Badge>}
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={script.isActive ? "default" : "secondary"}>
                    {script.isActive ? t("scripts.active") : t("scripts.inactive")}
                  </Badge>
                  <Button size="sm" variant="outline" onClick={() => executeScript(script.id)}>
                    <Play className="h-3 w-3 mr-1" />{t("scripts.run")}
                  </Button>
                </div>
              </div>
            </CardContent>
