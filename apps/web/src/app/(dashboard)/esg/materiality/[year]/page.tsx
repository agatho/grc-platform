...
function MaterialityYearInner() {
  ...
  const fetchVotes = useCallback(async (topicId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/v1/esg/materiality/topics/${topicId}/votes`,
      );
      if (res.ok) {
  ...
  const handleFinalize = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/esg/materiality/${year}/finalize`, {
        method: "POST",
      });
      if (res.ok) {
        await fetchData();
    ...
    return (
      <div className="space-y-4">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => router.push("/esg/materiality")}
        >
  ...
  const maxFinancial = Math.max(
    10,
    ...topics.map((tp) => tp.financialScore ?? 0),
  );
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="absolute inset-0">
            ...
            >
              ...
              {topics.map((tp) => {
                const x =
                  maxImpact > 0
                    ? ((tp.impactScore ?? 0) / maxImpact) * 95 + 2.5
                    : 50;
                const y =
                  maxFinancial > 0
                    ? 97.5 - ((tp.financialScore ?? 0) / maxFinancial) * 95
                    : 50;
                const r = Math.max(
                  1.5,
                  Math.min(4, (tp.stakeholderConsensus ?? 5) / 2.5),
                );
            ...
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">
                  {t("materiality.esrsStandard")}
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">
                  {t("materiality.topicName")}
                </th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500">
                  {t("materiality.impactScore")}
                </th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500">
                  {t("materiality.financialScore")}
                </th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500">
                  {t("materiality.consensus")}
                </th>
                <th className="px-4 py-3 text-center text-xs font-medium text-gray-500">
                  {t("materiality.isMaterial")}
                </th>
              </tr>
            </thead>
              ...
              {topics.map((tp) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right text-gray-700">
                    {tp.impactScore?.toFixed(1) ?? "-"}
                  </td>
                  <td className="px-4 py-3 text-right text-gray-700">
                    {tp.financialScore?.toFixed(1) ?? "-"}
                  </td>
                  <td className="px-4 py-3 text-right text-gray-700">
                    {tp.stakeholderConsensus?.toFixed(1) ?? "-"}
                  </td>
      ...
      {iroItems.filter((iro) => iro.iroType === "risk" && iro.isMaterial)
        .length > 0 && (
                    ...
                    <tr key={iro.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        {iro.ermRiskId ? (
                          <Badge
                            variant="outline"
                            className="text-blue-600 border-blue-200 text-[10px]"
                          >
                ...
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">
                      {t("materiality.voterName")}
                    </th>
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">
                      {t("materiality.voterType")}
                    </th>
                  ...
                  {votes.map((v) => (
                    <tr key={v.id}>
                      ...
                      <td className="px-4 py-2 text-right text-gray-700">
                        {Number(v.impactScore).toFixed(1)}
                      </td>
                      <td className="px-4 py-2 text-right text-gray-700">
                        {Number(v.financialScore).toFixed(1)}
                      </td>
