...
function MetricDetailInner() {
  ...
  const handleVerify = async (measurementId: string) => {
    try {
      await fetch(`/api/v1/esg/measurements/${measurementId}/verify`, {
        method: "POST",
      });
      await fetchData();
    } catch {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {t("metrics.detail")} | {metric.unit} |{" "}
              {metric.frequency.replace("_", " ")}
            </p>
          </div>
        </div>
              ...
              <LineChart data={chartData}>
                ...
                <Tooltip
                  ...
                  formatter={(val: unknown) =>
                    [`${val} ${metric.unit}`, t("value")] as const
                  }
                />
                <Line
                  type="monotone"
