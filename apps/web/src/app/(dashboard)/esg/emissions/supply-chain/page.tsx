...
const DEMO_REGIONS: HotspotRegion[] = [
  { region: "Asien-Pazifik", emissions: 12500, vendorCount: 23, riskLevel: "high" },
  { region: "Europa (EU)", emissions: 8200, vendorCount: 45, riskLevel: "medium" },
  { region: "Nordamerika", emissions: 5100, vendorCount: 12, riskLevel: "low" },
  { region: "Südamerika", emissions: 3200, vendorCount: 8, riskLevel: "medium" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Lieferketten-Emissions-Hotspots</h1>
          <p className="text-sm text-gray-500 mt-1">
            Scope 3 Emissionen nach Region und Kategorie — Identifikation der Haupttreiber
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{DEMO_REGIONS.length}</p>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{DEMO_REGIONS.reduce((s, r) => s + r.vendorCount, 0)}</p>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{(totalEmissions / 1000).toFixed(1)}k</p>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{DEMO_REGIONS.filter((r) => r.riskLevel === "high" || r.riskLevel === "critical").length}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {DEMO_REGIONS.sort((a, b) => b.emissions - a.emissions).map((region) => {
              const pct = Math.round((region.emissions / totalEmissions) * 100);
              return (
                <div key={region.region} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-gray-900">{region.region}</span>
                      <Badge variant="outline" className={`text-[10px] ${riskColors[region.riskLevel]}`}>
                        {region.riskLevel}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-gray-500">
                      <span>{region.vendorCount} Lieferanten</span>
                      <span className="font-mono font-medium text-gray-900">{region.emissions.toLocaleString("de-DE")} tCO₂e</span>
                    </div>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${
                        region.riskLevel === "critical" || region.riskLevel === "high" ? "bg-orange-500" : "bg-blue-500"
                      }`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
              ...
              <div key={cat.category} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    ...
                    <span className="font-mono font-medium text-gray-900">{cat.emissions.toLocaleString("de-DE")} tCO₂e</span>
                  </div>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2">
                  <div
