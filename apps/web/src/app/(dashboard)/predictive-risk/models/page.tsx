...
export default function PredictionModelsPage() {
  ...
  const trainModel = async (id: string) => {
    await fetch(`/api/v1/predictive-risk/models/${id}/train`, { method: "POST" });
    await fetchModels();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button><Plus className="h-4 w-4 mr-2" />{t("models.create")}</Button>
      </div>
        ...
        {models.map((model) => (
          <Card key={model.id}>
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base flex items-center gap-2"><Brain className="h-4 w-4" />{model.name}</CardTitle>
                <Badge className={STATUS_COLORS[model.status] ?? ""}>{model.status}</Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div className="text-sm text-muted-foreground space-y-1">
                <p>{t("models.type")}: {model.modelType}</p>
                <p>{t("models.algorithm")}: {model.algorithm}</p>
                <p>{t("models.target")}: {model.targetMetric}</p>
                {model.accuracy && <p>{t("models.accuracy")}: {Number(model.accuracy).toFixed(1)}%</p>}
                <p>{t("models.samples")}: {model.trainingSamples}</p>
              </div>
              <div className="flex gap-2 mt-3">
                <Button size="sm" variant="outline" onClick={() => trainModel(model.id)} disabled={model.status === "training"}>
                  <Play className="h-3 w-3 mr-1" />{t("models.train")}
                </Button>
              </div>
            </CardContent>
          </Card>
