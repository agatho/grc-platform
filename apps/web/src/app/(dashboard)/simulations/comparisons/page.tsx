  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t("comparisonsTitle")}</h1>
          <p className="text-sm text-gray-500 mt-1">{t("comparisonsDescription")}</p>
        </div>
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="text-center py-12 text-gray-400">{t("noComparisons")}</div>
      ) : (
          ...
          {items.map((c) => (
            <div key={c.id} className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400">({c.scenarioIds.length} {t("scenarios")})</span>
              </div>
