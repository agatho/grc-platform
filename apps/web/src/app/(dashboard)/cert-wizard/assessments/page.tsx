...
export default function CertAssessmentsPage() {
  ...
  >({
    queryKey: ["cert-wizard", "assessments"],
    queryFn: async () => {
      const res = await fetch("/api/v1/cert-wizard/assessments?limit=50");
      if (!res.ok) return [];
      return (await res.json()).data as CertReadinessAssessment[];
    },
  });
  if (loading)
    return (
      ...
      <div className="space-y-2">
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="font-medium">
                  {a.assessmentCode} - {a.title}
                </p>
                <p className="text-sm text-muted-foreground">
                  {a.framework} {a.frameworkVersion ?? ""} |{" "}
                  {a.targetCertDate ?? "No target date"}
                </p>
              </div>
              <div className="flex gap-2 items-center">
                <div className="text-right">
                  <p className="text-lg font-bold">
                    {a.readinessScore
                      ? `${Number(a.readinessScore).toFixed(0)}%`
                      : "N/A"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {a.implementedControls}/{a.totalControls}
                  </p>
                </div>
        ...
        {rows.length === 0 && (
          <p className="text-muted-foreground text-center py-8">
            No readiness assessments yet. Start your certification journey.
          </p>
        )}
      </div>
    </div>
