...
function ConsentDashboardInner() {
  ...
  const avgRate =
    types.length > 0
      ? types.reduce((a, t) => a + t.withdrawalRate, 0) / types.length
      : 0;
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{t("activeConsents")}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{t("totalWithdrawals")}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{t("avgWithdrawalRate")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div
              className={`text-3xl font-bold ${avgRate > 30 ? "text-red-600" : "text-green-600"}`}
            >
              {avgRate.toFixed(1)}%
            </div>
          </CardContent>
            ...
            <CardContent className="flex items-center gap-4 py-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {ct.purpose} / {ct.collectionPoint}
                </p>
              </div>
              <div className="text-right text-sm">
                <div>
                  {t("active")}: <strong>{ct.activeConsents}</strong>
                </div>
                <div
                  className={
                    ct.withdrawalRate > 30
                      ? "text-red-600"
              ...
              {ct.freelyGivenStatus && (
                <Badge
                  variant={
                    ct.freelyGivenStatus === "valid"
                      ? "default"
                      : ct.freelyGivenStatus === "questionable"
                        ? "secondary"
                        : "destructive"
                  }
                >
