      ...
      for (const sla of slas) {
        try {
          const res = await fetch(
            `/api/v1/contracts/${id}/sla/${String(sla.id)}/measurements`,
          );
          if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <Badge
          variant="outline"
          className={STATUS_COLORS[String(c.status)] ?? ""}
        >
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          ...
          <TabsTrigger value="obligations" className="gap-1.5">
            <ClipboardCheck size={14} /> {t("tab.obligations")} (
            {obligations.length})
          </TabsTrigger>
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-6">
              ...
              <dl className="space-y-3">
                <DetailRow
                  ...
                  value={t(`type.${String(c.contractType)}`)}
                />
                <DetailRow
                ...
                />
                <DetailRow
                  label={t("contract.effective")}
                  value={String(c.effectiveDate ?? "\u2014")}
                />
                <DetailRow
                  label={t("contract.expiration")}
                  value={String(c.expirationDate ?? "\u2014")}
                />
                <DetailRow
                  label={t("contract.noticePeriod")}
                  value={
                    c.noticePeriodDays
                      ? `${c.noticePeriodDays} ${t("days")}`
                      : "\u2014"
                  }
                />
                <DetailRow
                  label={t("contract.autoRenewal")}
                  value={c.autoRenewal ? t("yes") : t("no")}
                />
                <DetailRow
                  label={t("contract.owner")}
                  value={String(c.ownerName ?? "\u2014")}
                />
              </dl>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-6">
              ...
              <dl className="space-y-3">
                <DetailRow
                  ...
                  value={formatValue(c.totalValue as string)}
                />
                <DetailRow
                  label={t("contract.annualValue")}
                  value={formatValue(c.annualValue as string)}
                />
                <DetailRow
                  label={t("contract.currency")}
                  value={String(c.currency ?? "EUR")}
                />
                <DetailRow
                  label={t("contract.paymentTerms")}
                  value={String(c.paymentTerms ?? "\u2014")}
                />
                <DetailRow
                  label={t("contract.signed")}
                  value={String(c.signedDate ?? "\u2014")}
                />
              </dl>
            </div>
          </div>
              ...
              {obligations.map((o: Record<string, unknown>) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-xs">
                        {String(o.obligationType)}
                      </Badge>
                      <Badge
                        variant="outline"
                        className={`text-xs ${OBL_STATUS_COLORS[String(o.status)] ?? ""}`}
                      >
                  ...
                  <div className="flex items-center gap-4 mt-1 text-xs text-gray-500">
                    <span>
                      {t("obligation.due")}: {String(o.dueDate ?? "\u2014")}
                    </span>
                    {Boolean(o.responsibleName) && (
                      <span>
                        {t("obligation.responsible")}:{" "}
                        {String(o.responsibleName)}
                      </span>
                    )}
                    {Boolean(o.recurring) && (
                      <Badge variant="outline" className="text-xs">
                        {t("obligation.recurring")}
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-gray-900">
                        {String(sla.metricName)}
                      </span>
                      <div className="flex items-center gap-2 text-xs">
                        <span className="text-gray-500">
                          {t("sla.target")}: {String(sla.targetValue)}
                          {String(sla.unit)}
                        </span>
                        <Badge variant="outline">
                          {String(sla.measurementFrequency)}
                        </Badge>
                      </div>
                    </div>
                    {latest && (
                      <div
                        className={`flex items-center justify-between rounded px-3 py-1.5 text-xs ${latest.isBreach ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}
                      >
                        <span>
                          {t("sla.latest")}: {String(latest.actualValue)}
                          {String(sla.unit)}
                        </span>
                        <span>
                          {latest.isBreach ? t("sla.breach") : t("sla.ok")}
                        </span>
                    ...
                    {measurements.length > 1 && (
                      <p className="text-xs text-gray-400 mt-1">
                        +{measurements.length - 1} {t("sla.moreMeasurements")}
                      </p>
                    )}
                  </div>
                );
