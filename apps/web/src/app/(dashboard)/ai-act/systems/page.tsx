...
export default function AiSystemsPage() {
  ...
  const { data: rows = [], isPending: loading } = useQuery<AiSystem[]>({
    queryKey: ["ai-act", "systems"],
    queryFn: async () => {
      const res = await fetch("/api/v1/ai-act/systems?limit=50");
      if (!res.ok) return [];
      return (await res.json()).data as AiSystem[];
    },
  });
  if (loading)
    return (
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div>
                  <p className="font-medium">
                    {sys.systemCode} - {sys.name}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {sys.aiTechnique} | {sys.providerOrDeployer}
                  </p>
                </div>
                <div className="flex gap-2">
                  <Badge className={RISK_COLORS[sys.riskClassification] ?? ""}>
                    {sys.riskClassification}
                  </Badge>
