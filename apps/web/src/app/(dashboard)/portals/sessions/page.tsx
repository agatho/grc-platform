  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t("sessionsTitle")}</h1>
          <p className="text-sm text-gray-500 mt-1">{t("sessionsDescription")}</p>
        </div>
        ...
        <div className="space-y-3">
          {sessions.map((s) => (
            <div key={s.id} className="flex items-center justify-between rounded-lg border border-gray-200 bg-white p-4">
              <div>
                <span className="text-sm font-medium text-gray-900">{s.externalEmail}</span>
                {s.externalName && <span className="text-xs text-gray-500 ml-2">({s.externalName})</span>}
                {s.externalOrg && <span className="text-xs text-gray-500 ml-1">- {s.externalOrg}</span>}
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="outline" className={statusColors[s.status] ?? ""}>{s.status}</Badge>
                  {s.mfaVerified && <Badge variant="outline" className="text-[10px] bg-blue-50 text-blue-700">MFA</Badge>}
                  <span className="text-xs text-gray-400">{t("expiresAt")}: {new Date(s.expiresAt).toLocaleString()}</span>
                </div>
              </div>
              {s.status === "active" && (
