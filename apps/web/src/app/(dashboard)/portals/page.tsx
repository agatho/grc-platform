...
import { Loader2, RefreshCcw, Plus, Users, ShieldCheck, Eye, AlertTriangle } from "lucide-react";
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/portals/sessions">
            <Button variant="outline" size="sm">{t("viewSessions")}</Button>
          </Link>
          ...
          {configs.map((config) => (
            ...
            >
              <div className="flex items-center gap-3">
                {portalTypeIcons[config.portalType] ?? <Users size={18} className="text-gray-400" />}
                <div>
              ...
              {config.description && (
                <p className="text-xs text-gray-500 mt-3 line-clamp-2">{config.description}</p>
              )}
              <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
                <Badge variant="outline" className={config.isActive ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}>
                  {config.isActive ? t("active") : t("inactive")}
                </Badge>
                {config.requireMfa && (
                  <Badge variant="outline" className="bg-blue-50 text-blue-700">{t("mfaRequired")}</Badge>
                )}
              </div>
            </Link>
          ))}
