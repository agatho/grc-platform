...
export default function ExecutiveReportPage() {
  ...
  const handleExportPdf = async () => {
    try {
      const res = await fetch(
        `/api/v1/budget/report/${year}/export?format=pdf`,
      );
      if (res.ok) {
  ...
  const budgetChangePercent =
    d.previousYear && d.previousYear.totalActual > 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <SummaryCard
          label={t("report.totalBudget")}
          value={d.totalBudget}
          currency={t("currency")}
        />
        <SummaryCard
          label={t("report.totalActual")}
          value={d.totalActual}
          currency={t("currency")}
        />
        <SummaryCard
          label={t("report.totalForecast")}
          value={d.totalForecast}
          currency={t("currency")}
        />
        <SummaryCard
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {d.topInvestments.map((inv, idx) => (
                <tr
                  key={`${inv.entityType}-${inv.entityId}`}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right">
                    <span
                      className={`font-bold ${inv.roiPercent >= 100 ? "text-green-700" : "text-yellow-700"}`}
                    >
      ...
      <div className="rounded-lg border-2 border-red-200 bg-red-50 p-6">
        ...
        <div className="space-y-2">
          {d.roniHighlight.topRisks.map((risk, idx) => (
            <div
              key={idx}
              className="flex justify-between items-center bg-white rounded-lg border border-red-100 px-4 py-2"
            >
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500">
                  {t("report.expectedBenefit")}
                </th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500">
                  {t("report.roniIfDeferred")}
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
