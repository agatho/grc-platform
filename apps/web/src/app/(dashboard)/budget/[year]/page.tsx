...
export default function BudgetYearPage() {
  ...
  const updateCell = (
    ...
    setMatrix((prev) =>
      prev.map((c) =>
        c.area === area && c.category === category
          ? { ...c, [field]: value }
          : c,
      ),
    );
            ...
            <thead className="bg-gray-50">
              <tr>
                ...
                {COST_CATEGORIES.map((cat) => (
                  <th
                    key={cat}
                    className="px-4 py-3 text-right text-xs font-medium text-gray-500 min-w-[120px]"
                  >
                    {t(`categories.${cat}`)}
                  </th>
                ))}
                          ...
                          <td key={q} className="px-4 py-2 text-right">
                            {isDraft ? (
                              <input
                                ...
                                value={cell[q]}
                                onChange={(e) =>
                                  updateCell(area, cat, q, e.target.value)
                                }
          ...
          <td key={cat} className="px-4 py-3 text-right">
            {isDraft ? (
              <input
                ...
                value={val}
                onChange={(e) =>
                  updateCell(area, cat, "plannedAmount", e.target.value)
                }
