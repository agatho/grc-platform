  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t("contributionsTitle")}</h1>
          <p className="text-sm text-gray-500 mt-1">{t("contributionsDescription")}</p>
        </div>
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="text-center py-12 text-gray-400">{t("noContributions")}</div>
      ) : (
          ...
          {items.map((c) => (
            <div key={c.id} className="rounded-lg border border-gray-200 bg-white p-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                <Badge variant="outline" className={statusColors[c.status] ?? ""}>{c.status.replace(/_/g, " ")}</Badge>
                <Badge variant="outline" className="text-[10px]">{c.contributionType.replace(/_/g, " ")}</Badge>
                <span className="text-xs text-gray-400">{new Date(c.createdAt).toLocaleDateString()}</span>
              </div>
              {c.description && <p className="text-xs text-gray-500 mt-2 line-clamp-2">{c.description}</p>}
              {c.prUrl && (
                <a href={c.prUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-2 text-xs text-blue-600 hover:text-blue-800">
