...
const RESULT_COLORS: Record<string, string> = {
  ...
  not_effective: "bg-red-100 text-red-900",
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {ctrl.processArea} | {ctrl.controlType} | {ctrl.frequency}{" "}
                  {ctrl.keyControl ? "| KEY" : ""}
                </p>
              </div>
              <div className="flex gap-2">
                {ctrl.lastTestResult && (
                  <Badge className={RESULT_COLORS[ctrl.lastTestResult] ?? ""}>
                    {ctrl.lastTestResult}
                  </Badge>
                )}
