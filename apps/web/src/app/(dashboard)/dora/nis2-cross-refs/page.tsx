...
const COMP_COLORS: Record<string, string> = {
  ...
  partially_compliant: "bg-yellow-100 text-yellow-900",
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="font-medium">
                  DORA {ref.doraArticle}{" "}
                  {ref.nis2Article ? `/ NIS2 ${ref.nis2Article}` : ""}
                </p>
                <p className="text-sm text-muted-foreground">
                  {ref.overlapType}
                </p>
              </div>
              <Badge className={COMP_COLORS[ref.complianceStatus] ?? ""}>
