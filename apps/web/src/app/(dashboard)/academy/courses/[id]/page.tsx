...
function CourseDetail() {
  ...
  if (!course) return <div className="text-center py-12 text-gray-400">{t("notFound")}</div>;
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3">
          ...
          {course.isMandatory && <Badge className="bg-red-100 text-red-800">{t("mandatory")}</Badge>}
          <Badge variant="outline">{course.courseType.replace(/_/g, " ")}</Badge>
        </div>
        {course.description && <p className="text-sm text-gray-500 mt-2">{course.description}</p>}
        <div className="flex gap-4 mt-3 text-sm text-gray-500">
          <span className="flex items-center gap-1"><Clock size={14} /> {course.durationMinutes} min</span>
          <span className="flex items-center gap-1"><CheckCircle size={14} /> {t("passingScore")}: {course.passingScorePct}%</span>
          <span>{t("version")}: {course.version}</span>
        </div>
      </div>

      <div className="rounded-lg border border-gray-200 bg-white p-6">
        <h2 className="text-base font-semibold mb-4">{t("lessons.title")} ({lessons.length})</h2>
        {lessons.length === 0 ? (
          <p className="text-sm text-gray-400">{t("lessons.empty")}</p>
        ) : (
          <div className="space-y-2">
            {lessons.map((lesson, i) => (
              <div key={lesson.id} className="flex items-center justify-between rounded border border-gray-100 p-3">
                <div className="flex items-center gap-3">
