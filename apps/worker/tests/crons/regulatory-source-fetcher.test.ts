...
describe("processRegulatorySources", () => {
  beforeEach(() => {
    const m = resetMockDb();
  ...
  it("smoke: import and run without throwing", async () => {
    const mod: Record<string, unknown> = await import("../../src/crons/regulatory-source-fetcher");
    const fn = mod["processRegulatorySources"];
