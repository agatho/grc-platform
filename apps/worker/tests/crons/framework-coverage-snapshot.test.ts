...
describe("frameworkCoverageSnapshotJob", () => {
  beforeEach(() => {
    const m = resetMockDb();
  ...
  it("smoke: import and run without throwing", async () => {
    const mod: Record<string, unknown> = await import("../../src/crons/framework-coverage-snapshot");
    const fn = mod["frameworkCoverageSnapshotJob"];
