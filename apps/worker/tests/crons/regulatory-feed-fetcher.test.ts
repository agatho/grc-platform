...
describe("processRegulatoryFeedFetcher", () => {
  beforeEach(() => {
    const m = resetMockDb();
  ...
  it("smoke: import and run without throwing", async () => {
    const mod: Record<string, unknown> = await import("../../src/crons/regulatory-feed-fetcher");
    const fn = mod["processRegulatoryFeedFetcher"];
