...
describe("processPlaybookPhaseEscalation", () => {
  beforeEach(() => {
    const m = resetMockDb();
  ...
  it("smoke: import and run without throwing", async () => {
    const mod: Record<string, unknown> = await import("../../src/crons/playbook-phase-escalation");
    const fn = mod["processPlaybookPhaseEscalation"];
