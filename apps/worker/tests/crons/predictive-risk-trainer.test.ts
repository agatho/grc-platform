...
describe("processPredictiveRiskTrainer", () => {
  beforeEach(() => {
    const m = resetMockDb();
  ...
  it("smoke: import and run without throwing", async () => {
    const mod: Record<string, unknown> = await import("../../src/crons/predictive-risk-trainer");
    const fn = mod["processPredictiveRiskTrainer"];
