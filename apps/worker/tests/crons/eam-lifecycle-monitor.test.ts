...
describe("processEamLifecycleMonitor", () => {
  beforeEach(() => {
    const m = resetMockDb();
  ...
  it("smoke: import and run without throwing", async () => {
    const mod: Record<string, unknown> = await import("../../src/crons/eam-lifecycle-monitor");
    const fn = mod["processEamLifecycleMonitor"];
