...
export async function processCommunityLicenseCheck(): Promise<{
  ...
  const expired = await db.select().from(communityEditionConfig)
    .where(
      and(
  ...
  for (const config of expired) {
    try {
      ...
      await db.update(communityEditionConfig).set({
      ...
      downgradeCount++;
      console.log(`[community-license-check] Downgraded org ${config.orgId} to community edition`);
    } catch (err) {
      console.error(`[community-license-check] Failed to downgrade org ${config.orgId}:`, err);
    }
  }

  console.log(`[community-license-check] Found ${expired.length} expired, downgraded ${downgradeCount}`);
