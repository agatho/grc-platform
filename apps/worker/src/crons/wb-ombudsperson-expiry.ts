...
interface OmbudspersonExpiryResult { processed: number; deactivated: number; }
...
export async function processWbOmbudspersonExpiry(): Promise<OmbudspersonExpiryResult> {
  ...
  const expired = await db.update(wbOmbudspersonAssignment)
  ...
  console.log(`[cron:wb-ombudsperson-expiry] Deactivated ${expired.length} assignments`);
