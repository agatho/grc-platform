...
export async function processMiningConformance(): Promise<ConformanceResult> {
  ...
  const result = { processed: logs.length, analyzed, errors };
  console.log(
    `[cron:process-mining-conformance] Done: processed=${result.processed} analyzed=${result.analyzed} errors=${result.errors}`,
  );
  return result;
}
