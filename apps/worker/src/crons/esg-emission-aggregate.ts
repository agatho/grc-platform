...
export const processEsgEmissionAggregate = withCronInstrumentation(
  "esg-emission-aggregate",
  async (): Promise<EsgEmissionAggregateResult> => {
    const now = new Date();
    ...
    for (const _entry of orgSummaries.entries()) {
      aggregatedCount++;
    }
