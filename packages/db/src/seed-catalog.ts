...
async function seed() {
  ...
  if (existingRiskCatalog.length > 0) {
    ...
    console.log("  Cambridge v2 risk catalog already exists, skipping catalog insert.");
  } else {
  ...
  `);
  const l1Map = Object.fromEntries(level1Entries.map((e: any) => [e.code, e.id]));
  ...
  `);
  const l2Map = Object.fromEntries(level2Entries.map((e: any) => [e.code, e.id]));
  ...
  console.log(`  Cambridge v2 risk catalog: ${riskEntryCount[0].cnt} entries (5 L1, 15 L2, 30 L3)`);
  ...
  if (existingControlCatalog.length > 0) {
    ...
    console.log("  ISO 27002:2022 control catalog already exists, skipping catalog insert.");
  } else {
    const inserted = await db.execute(sql`
  ...
  console.log(`  ISO 27002:2022 control catalog: ${ctrlEntryCount[0].cnt} entries (4 L1, 20 L2)`);
