...
export const approvalDecision = pgTable("approval_decision", {
  id: uuid("id").primaryKey().defaultRandom(),
  requestId: uuid("request_id").notNull().references(() => approvalRequest.id, { onDelete: "cascade" }),
  stepNumber: integer("step_number").notNull(),
  approverId: uuid("approver_id").notNull().references(() => user.id),
...
export const reviewDecision = pgTable("review_decision", {
  id: uuid("id").primaryKey().defaultRandom(),
  cycleId: uuid("cycle_id").notNull().references(() => reviewCycle.id, { onDelete: "cascade" }),
...
export const attestationResponse = pgTable("attestation_response", {
  id: uuid("id").primaryKey().defaultRandom(),
  campaignId: uuid("campaign_id").notNull().references(() => attestationCampaign.id, { onDelete: "cascade" }),
