...
export const xbrlTag = pgTable("xbrl_tag", {
  id: uuid("id").primaryKey().defaultRandom(),
  taxonomyId: uuid("taxonomy_id").notNull().references(() => xbrlTaxonomy.id, { onDelete: "cascade" }),
...
export const xbrlTaggingInstance = pgTable("xbrl_tagging_instance", {
  ...
  taxonomyId: uuid("taxonomy_id").notNull().references(() => xbrlTaxonomy.id),
  tagId: uuid("tag_id").notNull().references(() => xbrlTag.id),
...
export const consolidationEntry = pgTable("consolidation_entry", {
  ...
  orgId: uuid("org_id").notNull().references(() => organization.id),
  entityId: uuid("entity_id").notNull().references(() => organization.id),
