...
('a1000000-0000-0000-0000-000000000001', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'machine_learning', 'high', 'Anhang III Nr. 4: KI-Systeme für Beschäftigung und Personalmanagement', 'annex_iii',
...
('a1000000-0000-0000-0000-000000000002', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'machine_learning', 'high', 'Anhang III Nr. 5b: KI für Kreditwürdigkeitsprüfung', 'annex_iii',
...
('a1000000-0000-0000-0000-000000000003', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'generative_ai', 'limited', 'Art. 50: Transparenzpflicht für KI-Systeme die mit Menschen interagieren', NULL,
...
('a1000000-0000-0000-0000-000000000004', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'deep_learning', 'minimal', 'Keine Hochrisiko-Kriterien erfüllt, kein direkter Einfluss auf Personen', NULL,
...
('a1000000-0000-0000-0000-000000000005', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'computer_vision', 'unacceptable', 'Art. 5(1)(f): Emotionsableitung am Arbeitsplatz potenziell verboten', NULL,
 'provider', 'Meridian Innovation Lab', 'Deutschland', NULL, true, 'under_review', false, 10)
ON CONFLICT (id) DO NOTHING;

-- ============================================================
-- 1 GPAI Model (Art. 51-56)
-- ============================================================
...
('a1000000-0000-0000-0000-a00000000001', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 45000, '1.2.0', '2025-11-01', 'registered',
 'Textgenerierung, Zusammenfassung, Klassifikation für GRC-Domäne. Deutsch und Englisch.',
 'Keine Echtzeitdaten, kein Internetzugang, keine multimodale Verarbeitung. Halluzinationsrate ~3% bei Fachfragen.',
 'Interner Einsatz für Compliance-Texterstellung, Risikoanalyse-Unterstützung und Dokumentenklassifikation')
ON CONFLICT (id) DO NOTHING;

-- ============================================================
-- 2 AI Incidents (Art. 62-63)
-- ============================================================
...
('a1000000-0000-0000-0000-b00000000001', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'critical', true,
 '2026-01-15 09:00:00+01', '2026-01-17 09:00:00+01',
 'contained',
 'Altersbias in Trainingsdaten: 85% der positiven Trainingsbeispiele waren Kandidaten unter 45 Jahren',
 'System sofort deaktiviert. Manuelle Nachbewertung aller betroffenen Bewerbungen. Retraining mit ausbalancierten Daten geplant.',
 120, 'discrimination'),
...
('a1000000-0000-0000-0000-b00000000002', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'medium', false,
 '2026-02-20 14:30:00+01', '2026-03-07 14:30:00+01',
 'resolved',
 'Falsche Gewährleistungsinformation im Prompt-Template: B2B/B2C-Unterscheidung fehlte',
 'Prompt-Template korrigiert, Factsheet für Gewährleistung als Retrieval-Quelle hinzugefügt',
 3, 'misinformation')
ON CONFLICT (id) DO NOTHING;

-- ============================================================
-- Prohibited Practice Screenings (Art. 5)
-- ============================================================
...
('a1e00000-0000-0000-0000-000000000002', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'System fällt unter verbotene Praktiken: Emotionsableitung am Arbeitsplatz',
 'withdrawal', true, 'critical', '2026-02-15', 'completed', true)
ON CONFLICT (id) DO NOTHING;

-- ============================================================
-- Authority Communication (Art. 73-78)
-- ============================================================
...
('a1f00000-0000-0000-0000-000000000001', 'c2446a5c-64f1-40a7-862a-8ab084f66f41',
 ...
 'Erstmeldung gemäß Art. 62 Abs. 1 AI Act. Schwerwiegender Vorfall mit potenzieller Grundrechtsverletzung (Diskriminierung). Details siehe Anlage.',
 'BNetzA-AI-2026-0042',
 'a1000000-0000-0000-0000-b00000000001',
 '2026-01-16 11:00:00+01', 'sent')
ON CONFLICT (id) DO NOTHING;
