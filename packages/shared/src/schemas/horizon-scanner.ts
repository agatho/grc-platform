...
export const createHorizonSourceSchema = z.object({
  ...
  sourceType: z.enum(["eu_oj", "bsi", "bafin", "enisa", "eba", "esma", "cert", "national_gazette", "custom"]),
  ...
  parserType: z.enum(["rss", "html_scraper", "api", "email", "manual"]).default("rss"),
...
export const horizonItemQuerySchema = z.object({
  page: z.coerce.number().int().min(1).default(1),
  limit: z.coerce.number().int().min(1).max(100).default(20),
  classification: z.enum(["critical", "high", "medium", "low", "informational"]).optional(),
  itemType: z.enum(["regulation", "directive", "guideline", "consultation", "enforcement", "standard", "alert"]).optional(),
  status: z.enum(["new", "triaged", "under_review", "assessed", "acknowledged", "dismissed"]).optional(),
  jurisdiction: z.string().max(100).optional(),
...
export const updateHorizonItemSchema = z.object({
  status: z.enum(["new", "triaged", "under_review", "assessed", "acknowledged", "dismissed"]),
});
...
export const createHorizonCalendarEventSchema = z.object({
  ...
  eventType: z.enum(["compliance_deadline", "enforcement_date", "consultation_end", "reporting_deadline", "transition_period"]),
  eventDate: z.string(),
  jurisdiction: z.string().max(100).optional(),
  framework: z.string().max(100).optional(),
