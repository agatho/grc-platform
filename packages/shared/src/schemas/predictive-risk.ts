...
export const createPredictionModelSchema = z.object({
  ...
  modelType: z.enum(["anomaly_detection", "trend_forecast", "correlation", "score_prediction", "early_warning"]),
  algorithm: z.enum(["arima", "prophet", "isolation_forest", "random_forest", "neural_net", "ensemble"]),
  targetMetric: z.enum(["risk_score", "kri_value", "incident_count", "control_effectiveness"]),
  inputFeatures: z.array(z.object({
...
export const updatePredictionModelSchema = createPredictionModelSchema.partial().omit({ modelType: true, algorithm: true });

export const predictionModelQuerySchema = z.object({
  page: z.coerce.number().int().min(1).default(1),
  limit: z.coerce.number().int().min(1).max(100).default(20),
  modelType: z.enum(["anomaly_detection", "trend_forecast", "correlation", "score_prediction", "early_warning"]).optional(),
  status: z.enum(["untrained", "training", "active", "degraded", "archived"]).optional(),
...
export const predictionQuerySchema = z.object({
  ...
  predictionType: z.enum(["score_forecast", "trend", "threshold_breach", "correlation"]).optional(),
});
...
export const anomalyQuerySchema = z.object({
  ...
  anomalyType: z.enum(["spike", "drop", "pattern_break", "drift", "outlier"]).optional(),
});
...
export const radarQuerySchema = z.object({
  ...
  entityTypes: z.array(z.enum(["risk", "kri", "control", "process"])).optional(),
