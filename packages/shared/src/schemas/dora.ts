...
export const updateDoraTlptPlanSchema = createDoraTlptPlanSchema
  .partial()
  .extend({
    status: z
      .enum(["draft", "planned", "in_progress", "completed", "remediation"])
      .optional(),
    findings: z
      .array(
        z.object({
...
export const updateDoraIctProviderSchema = createDoraIctProviderSchema
  .partial()
  .extend({
    complianceStatus: z
      ...
      .optional(),
    status: z
      .enum(["active", "under_review", "terminated", "pending"])
      .optional(),
  });
...
export const createDoraInfoSharingSchema = z.object({
  ...
  recipientGroups: z
    .array(
      z.object({ groupName: z.string(), contactEmails: z.array(z.string()) }),
    )
    .max(20)
    .optional(),
...
export const updateDoraNis2CrossRefSchema =
  createDoraNis2CrossRefSchema.partial();

export const doraNis2QuerySchema = z.object({
  page: z.coerce.number().int().min(1).default(1),
  limit: z.coerce.number().int().min(1).max(100).default(50),
  overlapType: z
    .enum(["full_overlap", "partial_overlap", "dora_only", "nis2_only"])
    .optional(),
  complianceStatus: z.string().max(20).optional(),
});
