...
// not just two letters — Cowork QA's failing example was "XX" which
// matches /^[A-Z]{2}$/ but is unassigned. Hard-code the canonical
// list (249 codes as of 2024-04 per ISO Online Browsing Platform);
// the standard updates at ~5-year cadence so a refresh is cheap.
// Uppercase coercion runs first so callers can send "de" or "De".
...
const isoCountryAlpha2 = z
  ...
  .toUpperCase()
  .pipe(
    z.string().refine((val) => ISO_3166_1_ALPHA2_CODES.has(val), {
      message:
