...
export const updateTaxCmsElementSchema = createTaxCmsElementSchema
  .partial()
  .extend({
    status: z
      .enum([
...
export const createTaxRiskSchema = z.object({
  ...
  affectedEntities: z
    .array(z.object({ entityId: z.string(), entityName: z.string() }))
    .max(50)
    .optional(),
...
export const createTaxIcfrControlSchema = z.object({
  ...
  assertion: z
    .enum(["existence", "completeness", "valuation", "rights", "presentation"])
    .optional(),
...
export const updateTaxIcfrControlSchema = createTaxIcfrControlSchema
  .partial()
  .extend({
    lastTestResult: z
      .enum(["effective", "partially_effective", "not_effective", "not_tested"])
      .optional(),
    status: z
...
export const updateTaxAuditPrepSchema = createTaxAuditPrepSchema
  .partial()
  .extend({
    documentChecklist: z
      .array(
    ...
    status: z
      .enum(["preparation", "active", "fieldwork", "closing", "completed"])
      .optional(),
  });
...
export const taxAuditPrepQuerySchema = z.object({
  ...
  limit: z.coerce.number().int().min(1).max(100).default(20),
  auditType: z
    .enum(["regular", "special", "follow_up", "vat_audit", "transfer_pricing"])
    .optional(),
  status: z
    .enum(["preparation", "active", "fieldwork", "closing", "completed"])
    .optional(),
