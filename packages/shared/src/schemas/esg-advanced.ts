...
export const updateEsgAdvMaterialityAssessmentSchema =
  createEsgAdvMaterialityAssessmentSchema.partial();
...
export const createIroSchema = z.object({
  ...
  valueChainStage: z
    .enum(["own_operations", "upstream", "downstream"])
    .optional(),
  timeHorizon: z.enum(["short_term", "medium_term", "long_term"]).optional(),
  financialMagnitude: z.enum(["low", "medium", "high", "very_high"]).optional(),
  financialLikelihood: z
    .enum(["remote", "possible", "likely", "very_likely"])
    .optional(),
  impactScale: z
    .enum(["limited", "moderate", "significant", "widespread"])
    .optional(),
  impactScope: z
    .enum(["limited", "moderate", "deep", "fundamental"])
    .optional(),
  impactIrremediable: z
    .enum(["reversible", "partially_reversible", "irremediable"])
    .optional(),
...
export const updateCollectionCampaignSchema =
  createCollectionCampaignSchema.partial();
...
export const updateEsrsDisclosureSchema = z.object({
  ...
  status: z
    .enum(["not_started", "in_progress", "draft", "reviewed", "final"])
    .optional(),
});
