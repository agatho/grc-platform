...
export function countCompletedProcedures(checklist: QmsProcedureChecklist): number {
...
export function assessQmsReadinessForCe(checklist: QmsProcedureChecklist): QmsReadinessResult {
  const maturityScore = computeQmsMaturity(checklist);
  const completed = countCompletedProcedures(checklist);
  const missing = (Object.keys(checklist) as Array<keyof QmsProcedureChecklist>).filter(
    (k) => !checklist[k],
  );
  ...
  let readyForCe = true;
  let reasoning = "Bereit fuer CE-Marking: alle 4 kritischen Procedures + >= 80 % Maturity.";
...
export function assessIso42001Gap(ctx: Iso42001Context): Iso42001GapResult {
  const controls: Array<[string, boolean, string]> = [
    ["4.1 AI-Act-RiskManagement = ISO 6.1", ctx.qms.riskManagementProcedure, "ISO 42001 6.1"],
    ["4.2 AI-Act-DataGov = ISO 8.2", ctx.qms.dataGovernanceProcedure, "ISO 42001 8.2"],
    ["4.3 AI-Act-TechDoc = ISO 7.5", ctx.qms.technicalDocumentationProcedure, "ISO 42001 7.5"],
    ["4.4 AI-Act-RecordKeeping = ISO 7.5", ctx.qms.recordKeepingProcedure, "ISO 42001 7.5"],
    ["4.5 AI-Act-Transparency = ISO 4.3", ctx.qms.transparencyProcedure, "ISO 42001 4.3"],
    ["4.6 AI-Act-Oversight = ISO 8.3", ctx.qms.humanOversightProcedure, "ISO 42001 8.3"],
    ["4.7 AI-Act-Accuracy = ISO 9.1", ctx.qms.accuracyRobustnessProcedure, "ISO 42001 9.1"],
    ["4.8 AI-Act-Cybersec = ISO 8.2", ctx.qms.cybersecurityProcedure, "ISO 42001 8.2"],
    ["4.9 AI-Act-Incident = ISO 10.1", ctx.qms.incidentReportingProcedure, "ISO 42001 10.1"],
    ["4.10 AI-Act-3rdParty = ISO 8.5", ctx.qms.thirdPartyManagementProcedure, "ISO 8.5"],
    ["5.1 ISO AI-Policy", ctx.hasAiPolicy, "ISO 42001 5.2"],
    ["5.2 ISO AI-Objectives", ctx.hasManagementObjectives, "ISO 42001 6.2"],
    ["5.3 ISO AI-Impact-Assessment", ctx.hasAiImpactAssessment, "ISO 42001 6.1.3"],
...
export function assessAiRiskPortfolio(risks: AiRisk[]): AiRiskAssessmentQuality {
