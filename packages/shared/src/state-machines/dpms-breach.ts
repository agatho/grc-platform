...
export function validateBreachGate9Assess(snapshot: BreachSnapshot): Blocker[] {
  ...
  if (!snapshot.title || snapshot.title.trim().length === 0) {
    blockers.push({ code: "missing_title", message: "Breach-Titel erforderlich.", gate: "G9", severity: "error" });
  }
...
export function validateBreachGate10NotifyDpa(snapshot: BreachSnapshot): Blocker[] {
  const blockers: Blocker[] = [];

  if (!snapshot.dataCategoriesAffected || snapshot.dataCategoriesAffected.length === 0) {
    blockers.push({
  ...
  if (snapshot.estimatedRecordsAffected === null || snapshot.estimatedRecordsAffected === undefined) {
    blockers.push({
      code: "missing_records_count",
      message: "Art. 33(3)(a): Anzahl betroffener Datensaetze (Schaetzung) erforderlich.",
      gate: "G10",
      severity: "error",
    });
  }
  if (!snapshot.containmentMeasures || snapshot.containmentMeasures.trim().length < 30) {
    blockers.push({
...
export function validateBreachGate11NotifyIndividuals(snapshot: BreachSnapshot): Blocker[] {
  const blockers: Blocker[] = [];

  if (!snapshot.dpaNotifiedAt) {
    blockers.push({
      code: "dpa_not_notified",
      message: "DPA-Meldung muss vor Individuen-Meldung erfolgen (guter Praxis).",
      gate: "G11",
      severity: "warning",
    });
  }
...
export function validateBreachGate12Close(snapshot: BreachSnapshot): Blocker[] {
  const blockers: Blocker[] = [];
  if (!snapshot.remediationMeasures || snapshot.remediationMeasures.trim().length < 50) {
    blockers.push({
  ...
  if (snapshot.isDpaNotificationRequired && !snapshot.dpaNotifiedAt) {
    blockers.push({
      ...
      message: "DPA-Notification erforderlich aber nicht erfolgt -- kann nicht geschlossen werden.",
      gate: "G12",
      severity: "error",
    });
  }
...
export function computeBreachDeadline(detectedAt: Date, now: Date = new Date()): BreachDeadline {
...
export function validateBreachTransition(req: BreachTransitionRequest): BreachTransitionResult {
  ...
  if (currentStatus === "notifying_dpa" && targetStatus === "notifying_individuals") {
