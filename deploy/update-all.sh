...
for svc in web worker; do
  ...
  if [ -n "$CUR_IMG" ]; then
    # [OP-273] The result of `docker tag` used to be discarded and the success
    # line printed regardless. For months that reported a web snapshot that
    # never existed: the running web image had lost its name (see step 5) and
    # could not be tagged. No snapshot means no rollback — say so.
    if ! docker tag "$CUR_IMG" "arctos-rollback/grc-${svc}:${OLD_COMMIT:0:12}" 2>/dev/null; then
      echo "  WARNING: rollback snapshot for '$svc' FAILED — image $CUR_IMG cannot be tagged."
      echo "           An automatic rollback of '$svc' will not be possible for this deploy."
      deploy_record "snapshot-failed" "$svc image ${CUR_IMG:0:19} not taggable"
      continue
    fi
...
# [OP-273] Tenants run EXACTLY the images step 2 built — no rebuild here.
#
# This step used to run `docker compose up -d --force-recreate --build` per
# tenant. The tenant compose declares `image: ghcr.io/arctos/grc-web:latest`
# plus its own `build:` block WITHOUT the GIT_SHA/GIT_BRANCH/BUILD_TIME build
# args, so every tenant rebuilt the application from scratch and moved the
# `:latest` tag onto that build. Three effects, all measured on 2026-09-15:
#   * the image the MAIN web container runs lost its name (`<none>`, 0B), so
#     the rollback snapshot of the next deploy could not tag it — no
#     `arctos-rollback/grc-web:*` image existed for any recent deploy;
#   * tenants ran a separately built artefact without its revision baked in,
#     never checked by the pilot-readiness step;
#   * tenant workers built their own images under per-tenant names.
#
# A `docker-compose.override.yml` in each tenant directory pins both services
# to the images step 2 built. Compose merges that file automatically, so a
# manual `docker compose up -d` in the tenant directory uses the same pins.
MAIN_IMAGES=$(docker compose -f "$COMPOSE_FILE" config --images 2>/dev/null || true)
MAIN_WEB_IMAGE=$(printf '%s\n' "$MAIN_IMAGES" | grep -i 'grc-web' | head -1 || true)
MAIN_WORKER_IMAGE=$(printf '%s\n' "$MAIN_IMAGES" | grep -i 'grc-worker' | head -1 || true)
if [ -d /opt/arctos/tenants ] && { [ -z "$MAIN_WEB_IMAGE" ] || [ -z "$MAIN_WORKER_IMAGE" ]; }; then
  abort "Main image names not resolvable from $COMPOSE_FILE (web='$MAIN_WEB_IMAGE', worker='$MAIN_WORKER_IMAGE') — tenants not restarted."
fi
if [ -d /opt/arctos/tenants ]; then
  echo "  Images: web=$MAIN_WEB_IMAGE worker=$MAIN_WORKER_IMAGE"
  for tdir in /opt/arctos/tenants/*/; do
    [ -d "$tdir" ] || continue
    TENANT=$(basename "$tdir")
    echo "  Tenant: $TENANT"
    cd "$tdir"
    PIN="${tdir%/}/docker-compose.override.yml"
    PIN_MARK="# ARCTOS image pin — generated by deploy/update-all.sh (OP-273)"
    if [ -f "$PIN" ] && ! head -1 "$PIN" | grep -qF "$PIN_MARK"; then
      echo "    WARNING: $PIN exists and was not written by this script — tenant not restarted."
      cd /opt/arctos
      continue
    fi
    {
      echo "$PIN_MARK"
      echo "# Do not edit: rewritten on every deploy."
      echo "services:"
      # Only pin services the tenant compose actually defines. A service that
      # exists ONLY in an override would be created as a new container.
      if grep -q "^  web-$TENANT:" docker-compose.yml; then
        printf '  web-%s:\n    image: %s\n' "$TENANT" "$MAIN_WEB_IMAGE"
      fi
      if grep -q "^  worker-$TENANT:" docker-compose.yml; then
        printf '  worker-%s:\n    image: %s\n' "$TENANT" "$MAIN_WORKER_IMAGE"
      fi
    } > "$PIN"
    docker compose up -d --force-recreate --no-build 2>&1 | tail -3

    # Worker-Health-Check pro Tenant: zeigt sofort ob die Cron-Engine für
    # diesen Tenant läuft oder im Crash-Loop steckt.
    sleep 3
    # [OP-273] `|| true`: without a match grep returns 1, and under
    # `set -euo pipefail` a failing assignment ends the deploy silently — the
    # OP-266 pattern. A tenant without a worker service would have hit it.
    WORKER_STATE=$(docker compose ps --format json "worker-$TENANT" 2>/dev/null | grep -oP '"State":"\K[^"]+' | head -1 || true)
