...
> **[Welle 6a · N-2] Nachtrag 2026-09-07: drei Pfade dazu, 19 → 22.**
> Das Einschalten von `noUnusedLocals` hat drei weitere Stellen freigelegt, die
> Erfolg meldeten, ohne etwas gemessen zu haben — erkennbar daran, dass sie
> ihre gesamte Fachlogik importierten und nicht benutzten. Sie verweigern
> jetzt nach demselben Muster wie die elf darüber. Belege und Messungen:
> `docs/UMSETZUNG-WELLE-6A.md` §5. Diese Datei wird hier nur deshalb
> angefasst, weil `apps/worker/tests/docs-vs-honest-refusals.test.ts` sie als
> Tor-Eingabe führt: eine neue Verweigerung im Code ohne Zeile hier lässt das
> Tor fallen — genau wie vorgesehen.

<!-- OP-104:refusal-paths=22 -->
...
| `apps/worker/src/crons/continuous-audit-runner.ts`             | wirft je `builtin`-Regel | Keine eingebaute Dauerprüfung; `executeBuiltinRule` gab `[]` zurück, und ein leeres Ergebnis wurde als `pass` persistiert                                                                                                                             |
| `apps/worker/src/crons/risk-prediction-weekly.ts`              | wirft, schreibt nichts   | Keine Eskalationsvorhersage; der Job lief wöchentlich und quittierte `success` mit `{processed:0}`                                                                                                                                                    |
| `apps/worker/src/crons/tech-radar-migration-alerts.ts`         | wirft, schreibt nichts   | Keine Migrationswarnung; `alertsCreated` trug die Zahl der KANDIDATEN, erzeugt wurde nie eine                                                                                                                                                         |
