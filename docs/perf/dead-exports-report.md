...
_Generated: 2026-09-02T20:27:36.549Z_
...
**2706 potenziell tote Exports** in 461 Dateien.
...
| `packages/shared/src/schemas/audit-advanced.ts`      | 20     | `createWpFolderSchema`, `updateWpFolderSchema`, `updateWorkingPaperSchema`, `WP_STATUS_TRANSITIONS`, `isValidWpTransition`, `wpTransitionSchema`, `createReviewNoteSchema`, `resolveReviewNoteSchema`, `createReviewNoteReplySchema`, `updateAuditorProfileSchema` ...                                                          |
...
- `apps/web/src/components/bpmn/grc-view-select.tsx` -- `GrcViewSelectProps`
...
- `apps/web/src/lib/api.ts` -- `checkCustomRoleModuleAccess`, `AuditAnnotation`, `DEFAULT_PAGE_SIZE`
...
- `apps/web/src/lib/grc-overlay.ts` -- `BuildOverlayOptions`
...
- `apps/web/src/lib/process-gates.ts` -- `GateBlocker`
...
- `packages/auth/src/providers.ts` -- `pickAttributableOrgId`, `LOGIN_MAX_FAILED_ATTEMPTS`, `LOGIN_LOCKOUT_MINUTES`, `LoginLockState`, `loadRoles`, `extractRequestInfo`, `isAzureAdConfigured`
- `packages/auth/src/rbac.ts` -- `requireLineOfDefense`, `getRolesInOrg`, `HINSCHG_ISOLATED_ROLES`, `isPublicExactOrUnder`
- `packages/auth/src/saml/index.ts` -- `parseSAMLMetadata`, `encodeAuthnRequestForRedirect`, `validateSAMLSignature`, `cleanupAssertionCache`, `rejectXXE`
...
- `packages/auth/src/saml/response-validator.ts` -- `cleanupAssertionCache`, `rejectXXE`, `SamlSignatureScope`, `VerifiedSamlResponse`, `validateSAMLSignature`
...
- `packages/db/src/schema/phase3-extras.ts` -- `catalogEntryMapping`, `evidenceRequest`, `inlineComment`, `messagingIntegration`, `moduleNavItem`, `reminderRule`, `soxScoping`, `tagDefinition`
...
- `packages/db/src/schema/process-diagram-grc.ts` -- `processLane`, `sodRule`, `processStepRaci`, `processStepRopa`, `processStepDataCategory`, `processStepRecipient`, `processStepBia`, `processStepDocument`, `processEventActivityMap`, `userDiagramPreference`
...
- `packages/shared/src/schemas/process.ts` -- `approvalStepTypeValues`, `generateBpmnSchema`, `commentListQuerySchema`
...
- `packages/shared/src/schemas/regulatory-change.ts` -- `updateRegulatoryCalendarEventSchema`
