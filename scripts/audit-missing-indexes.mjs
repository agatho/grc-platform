...
function extractTables(sql) {
  ...
  while ((m = ct.exec(sql)) !== null) {
    ...
    for (const p of parts) {
      if (
        /^\s*(CONSTRAINT|PRIMARY\s+KEY|UNIQUE|FOREIGN\s+KEY|CHECK|EXCLUDE)\b/i.test(
          p,
        )
      ) {
        const refs = p.match(
          /FOREIGN\s+KEY\s*\(([^)]+)\)\s+REFERENCES\s+["]?([a-z_]+)["]?/i,
        );
        if (refs) {
          const cols = refs[1]
            .split(",")
  ...
  const idx =
    /CREATE\s+(?:UNIQUE\s+)?INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?["]?[a-z_][a-z0-9_]*["]?\s+ON\s+["]?([a-z_][a-z0-9_]*)["]?\s*(?:USING\s+\w+\s*)?\(([^)]+)\)/gi;
  let im;
...
function suggestIndexes(tables) {
  ...
  for (const [table, t] of tables) {
    for (const col of t.columns) {
      const hasIdx = [...t.indexes].some(
        (i) => i === col.name || i.startsWith(col.name + ","),
      );
      if (hasIdx) continue;
      ...
      if (col.refs) reason = `FK -> ${col.refs}`;
        ...
        reason = "Timestamp-Sort";
      else if (col.name === "status" || col.name === "state")
...
async function main() {
  const sql =
    (await readAllSql(DRIZZLE_DIR)) + (await readAllSql(MIGRATIONS_DIR));
  ...
  md.push(``);
  md.push(
    `**${tables.size} Tabellen analysiert, ${suggestions.length} Index-Kandidaten.**`,
  );
  md.push(``);
  ...
  for (const [reason, items] of [...byReason.entries()].sort()) {
    ...
    for (const s of items.slice(0, 200)) {
      md.push(
        `CREATE INDEX IF NOT EXISTS idx_${s.table}_${s.column} ON ${s.table}(${s.column});`,
      );
    }
